...
- [ ] Array length = 3 (Phase 1), 5 (Phase 2), 7 (Phase 3) — **OR 6 for W16+**
...
- [ ] **W16+: Each `word` field is a MULTI-WORD collocation (contains space — see C8)**
...
### C8. Word Power `word` Field = Single Word, Không Phải Collocation (W16+)

**Nguồn:** Week 19  
**Mức độ:** 🔴 CRITICAL — Violates Blueprint collocation rule

**Triệu chứng:**
- Word Power station shows single vocabulary words (e.g. "kindergarten") instead of phrases
- Looks identical to Vocab station — no pedagogical value
- Users confused: Word Power is supposed to teach collocations, not isolated words

**Nguyên nhân gốc:**
Agent populated `word` fields by copying vocabulary words from vocab.js directly:

```javascript
// ❌ WRONG (single word copied from vocab.js):
{ id: 1, word: "kindergarten", ... }
{ id: 2, word: "grow", ... }

// ✅ CORRECT (collocation phrase):
{ id: 1, word: "go to kindergarten", ... }
{ id: 2, word: "grow up", ... }
```

**W19 Mapping (for reference):**
| Vocab Word | ❌ Wrong | ✅ Correct Collocation |
|-----------|---------|---------------------|
| kindergarten | "kindergarten" | "go to kindergarten" |
| grow | "grow" | "grow up" |
| past | "past" | "in the past" |
| young | "young" | "young child" |
| album | "album" | "photo album" |
| memory | "memory" | "childhood memory" |

**Phòng tránh:**

1. **Validate bằng space check:**
```bash
...
2. **CHECK 20 trong code_quality_gate.sh** tự động bắt lỗi này từ W19 trở đi.

3. **Collocation creation rule:**
   - Noun → add article or verb: "album" → "photo album" or "make an album"
   - Verb → add particle or object: "grow" → "grow up" or "grow plants"
   - Adjective → add noun: "young" → "young child" or "young student"

**Checklist:**
- [ ] W16+: Tất cả `word` fields có ít nhất 1 khoảng trắng
- [ ] Collocations lấy từ danh sách vocab (không tự tạo)
- [ ] Đú́ng 6 entries cho W16+

---

### C9. Easy Mode Tạo Thư Mục Ảnh Riêng (`weekN_easy/`) Khi Vocab Đồng Nhất (W16+)

**Nguồn:** Week 19  
**Mức độ:** 🟠 MEDIUM — Redundant work, doubles image storage

**Triệu chứng:**
- Hai bộ ảnh giống nhau tồn tại trên R2: `/images/weekN/` và `/images/weekN_easy/`
- Tốn thời gian tạo prompts + generate images cho easy mode
- Easy `vocab.js` trỏ đến `/images/weekN_easy/` thay vì `/images/weekN/`

**Nguyên nhân gốc:**
W16+ Advanced và Easy dùng cùng 13+ vocab words. Không cần ảnh riêng. Agent không biết rule này và tự động tạo `weekN_easy/` folder.

**Phòng tránh:**

**SHARED IMAGE MODE (W16+):**
- Nếu Advanced và Easy dùng cùng vocab list → **dùng chung 1 bộ ảnh**
- Tất cả Easy JS files (đặc biệt vocab.js, word_power.js) dùng paths `/images/weekN/`
- **Không tạo** `public/images/weekN_easy/` folder
- **Không tạo** `week_N_easy_image_prompts.txt` — 1 prompt file duy nhất
- **Audio vẫn riêng:** `/audio/weekN_easy/` (definitions khác nhau giữa 2 modes)

**Validation:**
```bash
# Easy vocab.js phải không có weekN_easy trong image_url
grep 'image_url' src/data/weeks_easy/week_N/vocab.js | grep 'weekN_easy'
# Output MUST be EMPTY
```

**CHECK 25 trong code_quality_gate.sh** tự động kiểm tra từ W19 trở đi.

**Checklist:**
- [ ] W16+: Easy `vocab.js` image_url không có `weekN_easy`
- [ ] 1 prompt file duy nhất (`week_N_image_prompts.txt`)
- [ ] Upload chỉ đến `/images/weekN/` trên R2
...
| **A5** | **AI Tutor content không update từ clone** | **Execution** | **🔴 Critical** | **W15** |
| B1 | Thiếu index.js | Structure | 🔴 Critical | W13 |
| B2 | Thiếu UI imports (3 files) | Structure | 🔴 Critical | W12, **W20** |
...
| **B7** | **`git status` trước push \u2014 files uncommitted** | **Structure** | **🟠 High** | **W20** |
| C1 | Easy mode = copy Advanced | Content | 🟠 High | W12 |
| C2 | Dictation/shadowing không copy từ read.js | Content | 🟠 High | W12 |
| C3 | Daily Watch videos không chạy/sai | Content | 🔴 Critical | W13, W15, **W20** |
...
| **C10** | **Word Power `word` = single word (W16+)** | **Content** | **🔴 Critical** | **W19** |
| **C11** | **Easy mode tạo thư mục ảnh riêng weekN_easy** | **Content** | **🟠 High** | **W19** |
| **C12** | **Vocab chỉ 10 từ thay vì 13 (W16+)** | **Content** | **🔴 Critical** | **W20** |
| **C13** | **conversation_cards thiếu trong AI Tutor** | **Content** | **🔴 Critical** | **W20** |
| **C14** | **image_url vocab.js không tồn tại trên disk** | **Content** | **🟠 High** | **W20** |
| **C15** | **Ảnh generate sai thứ tự → sai mapping** | **Content** | **🟠 High** | **W20** |
| **C16** | **Video ID trùng với tuần trước** | **Content** | **🟠 High** | **W20** |
...
**Tổng: 17 Critical, 13 High, 6 Medium = 36 lỗi known** *(Updated March 27, 2026 — Post W20 Audit)*
...
**Week 19 All Bugs:**
- **C10:** Word Power `word` field = single word (e.g. "kindergarten") not collocation → Quality Gate CHECK 20
- **C11:** Easy mode created separate `weekN_easy/` image folder instead of sharing `/weekN/` → Quality Gate CHECK 25

**Week 19 Impact:** 2 bugs (both new, W16+ specific)

**Week 20 All Bugs:**
- **B2 (recurrence):** 3 UI files uncommitted (FreeTalkTab, StoryMissionTab, gameAdaptation)
- **B7 (new):** No `git status` check before push → 3 files missed
- **C3 (recurrence):** Daily Watch duplicate/broken video IDs
- **C12 (new):** Vocab count 10 instead of 13 (W16+ requires 13+)
- **C13 (new):** conversation_cards missing in week_20_real.js → Quality Gate CHECK 31
- **C14 (new):** vocab.js image_url paths don't exist on disk → Quality Gate CHECK 32
- **C15 (new):** Images generated out of order 1→26 → auto_rename.py maps by position → wrong images
- **C16 (new):** Video ID duplicated from previous week → Quality Gate CHECK 33

**Week 20 Impact:** 8 bugs (5 new, 3 recurrences) → Added Quality Gate checks 31-34
...
## BUG-18 — Wrap ALL image `src` with `getImageUrl()` (March 2026)

- **Tuần phát hiện:** W16
- **Triệu chứng:** `<img src={item.path}>` trực tiếp → 404 production vì images nằm trên R2 CDN
- **Nguyên nhân:** Thiếu `import { getImageUrl }` và wrap `src={getImageUrl(item.path)}`
- **Fix:** Mọi `<img>` từ data file MUST dùng `src={getImageUrl(item.path)}`
- **Gate:** CHECK 39 trong `code_quality_gate.sh` — grep `src={[^g]` trong JSX → phải = 0 results

---

## BUG-19 — Dùng đúng station name khi gọi `speakText()` (March 2026)

- **Tuần phát hiện:** W16
- **Triệu chứng:** MindMap dùng `station='read'` thay vì `'mindmap_speaking'` → sai giọng, không dùng R2 CDN, fallback browser TTS
- **Nguyên nhân:** Copy-paste `speakText()` call từ component khác mà không kiểm tra lại station string
- **Fix:** Đối chiếu với `STATION_VOICE_KEY` trong `src/services/voiceService.js` trước khi dùng
- **Gate:** Manual review — grep station name sau khi tạo component mới

---

## BUG-20 — Cross-week vocab contamination khi copy template (March 2026)

- **Tuần phát hiện:** W22
- **Triệu chứng:** W22 read.js dùng W21 vocab (walked, cooked, played) → dictation+shadowing tự động sai theo
- **Nguyên nhân:** Agent copy read.js từ W(N-1) làm template, thay title/metadata nhưng KHÔNG rewrite story content
- **Fix:** Sau khi viết read.js, grep 3-5 từ vocab của tuần cũ → nếu xuất hiện là contamination
- **Gate:** CHECK 36 — tự động kiểm tra >= 50% vocab words của tuần N có mặt trong read.js

---

## BUG-21 — Easy mode vocab count phải bằng Advanced (March 2026)

- **Tuần phát hiện:** W22
- **Triệu chứng:** W22 easy vocab.js có 10 từ, advanced có 13 → easy mode thiếu 3 từ trên toàn bộ app
...
## BUG-22 — Grammar tense inconsistency trong read.js (March 2026)

- **Tuần phát hiện:** W22
- **Triệu chứng:** W22 easy read.js dùng present tense ("She walks") → dictation+shadowing tự động sai tense theo
- **Nguyên nhân:** Easy mode read.js copy từ W21 (present tense week) không đổi sang Simple Past
- **Fix:** Kiểm tra grammar_focus TRƯỚC khi viết read.js — nếu là past tense week, toàn bộ story dùng -ed verbs
- **Gate:** CHECK 38 — nếu grammar_focus chứa "past", kiểm tra read.js có >= 3 -ed forms

---

## BUG-23 — Explore.js không phải CLIL — viết sai format (March 2026)

- **Tuần phát hiện:** W21, W22
- **Triệu chứng:** explore.js là bài grammar exercise ("Choose the correct -ed form...") thay vì CLIL non-fiction article
...
- **Ví dụ đúng:** W22: "Forensic Science: How Real Detectives Solve Cases" | W21: "Science Field Journals" (Darwin, Jane Goodall)

---

## BUG-24 — VideoChallenge Write tab scroll bug + blank textarea on "Add to script" (April 2026)

- **Tuần phát hiện:** W30
- **Triệu chứng 1:** Khi có nhiều sentence frames, textarea bị height = 0 → không nhìn thấy ô gõ
- **Triệu chứng 2:** "Add to script" button copies `___` raw blanks (hoặc trống) vào textarea thay vì text đã điền
- **Nguyên nhân 1:** Container dùng `flex-col overflow-hidden` + textarea có `h-full` → khi frames render thêm height, textarea bị squeeze xuống 0
- **Nguyên nhân 2:** Button không kiểm tra blanks đã fill hay chưa — copy text ngay lập tức dù blank còn trống
- **Fix:** Container → `overflow-y-auto`; textarea → `min-h-[160px]` (remove `h-full`); button disabled cho đến khi ALL blanks có value; copy chỉ assembled clean text (replace `___` bằng filled values)
- **Gate:** CHECK Browser — VideoChallenge Write tab: (1) scroll works with 8+ frames, (2) "Add to script" disabled when blanks empty, (3) clean text copies when all filled
- **File fixed:** `src/modules/video/VideoChallenge.jsx`

---

## BUG-25 — Nova AI feedback trả về `{"type":"object"}` thay vì text (April 2026)

- **Tuần phát hiện:** W30 (retroactively affects W1-W29 too)
- **Triệu chứng:** Nova feedback box hiện `{"type":"object"}` hoặc JSON schema stub thay vì câu nhận xét bằng tiếng Anh
- **Nguyên nhân:** Backend Railway proxy bật JSON mode cứng; system prompt không chỉ định field `ai_response` → model trả về JSON schema stub `{"type":"object"}` thay vì content thật
- **Fix 1:** `skipGrammarGuard: true` trong `generateConversation()` call để bypass pre-filter
- **Fix 2:** System prompt thêm: `Respond in JSON: {"ai_response": "..."}` với example
- **Fix 3:** `aiProxy.js` defensive parse — nếu `clean` starts với `{` hoặc `[` nhưng không có `ai_response` field → trả về `'Good! Tell me more.'` thay vì JSON garbage
- **Gate:** Manually test VideoChallenge Speak tab → submit text → Nova phải trả về 1-2 câu natural English ≤40 words (NOT JSON)
- **File fixed:** `src/services/aiProxy.js`, `src/modules/video/VideoChallenge.jsx`

---

## BUG-26 — sentence_frames thiếu hoặc không đủ → học sinh không làm (Pedagogical, April 2026)

- **Tuần phát hiện:** W1-W30 audit
- **Triệu chứng:** Học sinh không viết gì cả trong VideoChallenge Write tab vì không biết bắt đầu từ đâu
- **Nguyên nhân:** Trước April 2026, tất cả 30 tuần chỉ có 4 frames bất kể độ dài script yêu cầu
- **Fix:** Tăng dần số frames theo chiến lược: W1-5=4, W6-10=5, W11-20=6, W21-25=7, W26-30=8 (commit `dbefc27`). Frames phải cover TẤT CẢ questions trong `prompt_en`.
- **Nguyên lý sư phạm:** *"thà mồi sẵn rồi HS làm còn hơn là HS không làm gì cả"* — dù HS lười, nếu có frame sẵn thì vẫn điền vào → output có
- **Gate:** CHECK 49-50 — số frames ≥ số question marks trong prompt_en; W31+ min 8 frames
- **Files fixed:** `src/data/weeks/week_*/writing.js` (30 files) + `src/data/weeks_easy/week_*/writing.js` (30 files)

---

## BUG-27 — Grammar schema: `correct` vs `answer` field naming (April 2026)

- **Tuần phát hiện:** W24-W30 (comprehensive audit, commit `fbeb9d5`)
- **Triệu chứng:** Grammar station W24-W30 không bao giờ nhận đúng đáp án — học sinh gõ đúng vẫn bị mark wrong
- **Nguyên nhân:** W1-W23 grammar.js dùng `answer:` nhưng W24-W30 dùng `correct:` — GrammarEngine chỉ đọc `currentQ.answer` → undefined → mọi input đều sai
- **Fix:** GrammarEngine đọc `currentQ.answer || currentQ.correct`; File: `src/modules/grammar/GrammarEngine.jsx`
- **Quy tắc bắt buộc từ W31+:** LUÔN dùng `answer:` (KHÔNG dùng `correct:`) trong grammar.js exercises
- **Gate:** `grep -c '"correct":' src/data/weeks/week_N/grammar.js` phải = 0 — nếu > 0 → rename sang `answer:`

---

## BUG-28 — Grammar unscramble: thiếu `words` array (April 2026)

- **Tuần phát hiện:** W13-W14, W18 easy, W24-W30 (comprehensive audit)
- **Triệu chứng:** Grammar station crash/trắng ở exercises type `unscramble`
- **Nguyên nhân:** Một số tuần dùng format cũ `[w1 / w2 / w3]` nhúng trong question string, KHÔNG có `words: [...]` array — GrammarEngine gọi `currentQ.words.map(...)` → crash TypeError
- **Fix engine:** IIFE parse bracket format: `question?.match(/\[([^\]]+)\]/)?.[1]?.split('/').map(w=>w.trim())`; File: `src/modules/grammar/GrammarEngine.jsx`
- **Quy tắc bắt buộc từ W31+:** Unscramble exercises BẮT BUỘC có `words: ["word1", "word2", ...]` array riêng — KHÔNG nhúng vào question string
- **Gate:** `node -e "import('./src/data/weeks/week_N/grammar.js').then(m => { const u=m.default.exercises.filter(e=>e.type==='unscramble'); u.forEach(e=>{ if(!e.words) throw new Error('Missing words array: '+JSON.stringify(e)) }) ; console.log('✅ All unscramble have words array') })"` 

---

## BUG-29 — Grammar unscramble: thiếu `question` field (April 2026)

- **Tuần phát hiện:** W13-W14 (comprehensive audit)
- **Triệu chứng:** Unscramble card không hiện câu hỏi — chỗ question trống
- **Nguyên nhân:** Exercises có `words` + `answer` nhưng KHÔNG có `question` field — GrammarEngine render `undefined`
- **Fix engine:** Fallback text: `currentQ.question || (currentQ.type === 'unscramble' ? 'Unscramble the words:' : '')`
- **Quy tắc bắt buộc:** LUÔN có `question: "Unscramble the words:"` (hoặc custom prompt) trong mọi unscramble exercise
- **Gate:** `grep -c '"question"' src/data/weeks/week_N/grammar.js` phải = số exercise có type unscramble

---

## BUG-30 — Grammar: `answer: ""` rỗng (April 2026)

- **Tuần phát hiện:** W12 easy (comprehensive audit)
- **Triệu chứng:** 8 grammar exercises luôn mark wrong dù học sinh gõ gì — vì target answer là chuỗi rỗng
- **Nguyên nhân:** Template/clone bị để `answer: ""` placeholder mà không điền
- **Exercises bị ảnh hưởng:** id:2,4,6,8,12,14,16,20 (cả 8 điều nhau `answer: ""`)
- **Fix:** Điền đáp án thực: can/run/jump/draw/swim/bark, v.v.
- **Quy tắc bắt buộc:** KHÔNG BAO GIỜ để `answer: ""` trong grammar.js — luôn điền đáp án thật
- **Gate:** `grep -c '"answer": ""' src/data/weeks/week_N/grammar.js` phải = 0

---

## BUG-31 — Explore MCQ: `correct_answer: "B"` không tương thích với engine (April 2026)

- **Tuần phát hiện:** W20 adv + easy (comprehensive audit)
- **Triệu chứng:** Explore check_questions luôn sai — học sinh chọn đúng vẫn bị mark wrong
- **Nguyên nhân:** W20 explore dùng MCQ format với `correct_answer: "B"` (chữ cái) — Explore engine đọc `q.answer || q.answer_en || []` → không tìm thấy field `answer` → mọi input đều sai
- **Fix data:** Thêm `answer: ["text option A/B/C", "paraphrase"]` vào mỗi check_question
- **Quy tắc bắt buộc:** LUÔN có `answer: [...]` array trong mọi check_question — kể cả khi có `correct_answer` letter. Array phải chứa text thật (không chỉ chữ cái "B")
- **Gate:** `node -e "import('./src/data/weeks/week_N/explore.js').then(m => { m.default.check_questions.forEach((q,i)=>{ if(!q.answer||!q.answer.length) throw new Error('q['+i+'] missing answer array') }); console.log('✅ All check_questions have answer array') })"` 

---

## BUG-32 — Read/Explore: string `"A / B"` thay vì array `["A", "B"]` (April 2026)

- **Tuần phát hiện:** W07 adv read (comprehensive audit)
- **Triệu chứng:** Q1/Q2 read luôn sai dù học sinh trả lời đúng theo cách nào
- **Nguyên nhân:** `answer: "Next to the pen / In the pencil case"` — engine treat toàn bộ string là 1 target. `academic` mode 60% word overlap chỉ match khi học sinh gõ gần như toàn bộ string → fail với partial answers
- **Fix:** Đổi thành `answer: ["Next to the pen", "In the pencil case"]`
- **Quy tắc bắt buộc:** KHÔNG BAO GIỜ dùng `"option1 / option2"` string format — LUÔN dùng array `["option1", "option2"]`
- **Gate:** `grep -n '"answer":' src/data/weeks/week_N/read.js | grep ' / '` phải = rỗng (0 kết quả)

---

## BUG-33 — Explore: thiếu synonym trong answer array (April 2026)

- **Tuần phát hiện:** W10 adv + easy explore (comprehensive audit)
- **Triệu chứng:** Học sinh gõ "countryside" → mark wrong vì answer chỉ có "farm"
- **Nguyên nhân:** Content dùng "countryside" nhưng answer array chỉ liệt kê "farm" — wordOverlap engine check xem answer array có match với content_en, nhưng cũng dùng answer array là target để check user input
- **Fix:** Thêm `"countryside"` vào answer array bên cạnh `"farm"`
- **Quy tắc bắt buộc:** Answer array PHẢI bao gồm TẤT CẢ synonyms/paraphrases xuất hiện trong content_en cho cùng 1 concept. Đặc biệt: British vs American English, compound words (seashore/seaside/coast), informal vs formal
- **Gate:** Chạy `node comprehensive_audit.mjs` → 0 ISSUES (file này đã có ở root workspace)

---

## BUG-34 — Audit: curly quotes không được normalize → false failures (April 2026)

- **Tuần phát hiện:** W21 adv read q3 (comprehensive audit)
- **Triệu chứng:** Answer `"nine o clock"` không match content `"nine o'clock"` (có apostrophe cong)
- **Nguyên nhân:** `wordOverlap()` normalize chỉ strip ASCII `'` nhưng content thường có curly quotes `'` `'` (Unicode U+2018/U+2019) từ copy-paste — `o'clock` → `oclock` sau strip → không match `oclock` từ `"o clock"`
- **Fix engine:** `normalize` function strip cả `[\u2018\u2019\u201C\u201D]` trước các ASCII punctuation; File: `comprehensive_audit.mjs`
- **Fix data:** Thêm `"nine o'clock"` (dấu nháy thật) vào answer array W21 adv read q3
- **Quy tắc bắt buộc:** Khi viết answer arrays, LUÔN dùng ASCII apostrophe `'` (U+0027) — KHÔNG dùng curly `'` (U+2018/U+2019). Audit tool tự handle, nhưng data nên canonical ASCII
- **Gate:** `grep -rP "[\u2018\u2019\u201C\u201D]" src/data/weeks/week_N/` phải = rỗng trong answer arrays
