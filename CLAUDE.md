...
14. **Transcript file separation** (Aug 1, 2026): When editing shadowing transcript, ONLY modify `src/data/video_transcripts_by_id/sentences/<videoId>.json`. NEVER modify `shadowing.js` content_en or script[] fields. The shadowing.js file is READ-ONLY for transcript work.
15. **NEVER use Deepgram or alignment tools on shadowing transcript** (Aug 2, 2026): YouTube auto-caption timestamps are the source of truth. Deepgram alignment broke timestamps in W10 (EfD2k9beP-4) and W31 (-0zVS6aDPXY). When doing manual sentence splitting, keep YouTube timestamps as-is. `cleaned/` files serve as fallback with reliable YouTube timestamps. **Source:** Audit 2026-08-02 — Deepgram provided no measurable improvement but created critical timestamp corruption.
