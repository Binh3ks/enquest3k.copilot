...
Phases:
  0  Setup & Clone
  1  Read & Explore
  2  Dictation & Shadowing
  3  Vocab, Grammar, Logic, Word Power, Writing
  4  Index.js & Daily Watch
  5  AI Tutor Customization
  6  Teacher Panel Lesson Plan (parse DOCX → inject 5 JSON files)
"""
...
# ============================================
# PHASE 6: TEACHER PANEL LESSON PLAN
# ============================================

def phase6_lesson_plan(week_num):
    """
    Parse W{N} from the reference DOCX (W1-W53) or generate from Syllabus V5 (W54+)
    and inject into all 5 required app files:
    ...
    W1-W53:  parsed from reference DOCX (0. NEW_FINAL_Lesson plans_W*.docx)
    W54-W156: generated from _syllabus_v5_raw.txt via _gen_lesson_from_syllabus.py
    W28 is the golden standard schema for all weeks.
    """
    print("\n" + "="*60)
    print(f"📋 PHASE 6: Teacher Panel Lesson Plan - Week {week_num}")
    print("="*60)

    root = Path(__file__).parent
    sys.path.insert(0, str(root))

    # ── Route: W54+ → Syllabus-based generator ──────────────────────────────
    if week_num >= 54:
        print(f"  Source: Syllabus V5 (_syllabus_v5_raw.txt)")
        try:
            import _gen_lesson_from_syllabus as gen
        except ImportError as e:
            print(f"❌ ERROR: Cannot import _gen_lesson_from_syllabus: {e}")
            return False

        parser = gen.SyllabusParser()
        builder = gen.LessonBuilder()

        week_data = parser.get_week(week_num)
        if not week_data:
            print(f"❌ ERROR: W{week_num} not found in Syllabus V5.")
            return False

        data = builder.build(week_data)
        gen.inject_lesson(week_num, data)

        tc = data.get('teacher_contents', [])
        warnings = gen.validate_lesson(week_num, data)
        todo_count = sum(1 for w in warnings if 'FILL-IN' in w)
        other_warns = [w for w in warnings if 'FILL-IN' not in w]

        if other_warns:
            print(f"\n⚠️  WARN: {'; '.join(other_warns)}")

        print(f"\n✅ PHASE 6 COMPLETE — W{week_num} injected into 5 files")
        print(f"   unit_theme: {data.get('unit_theme','')}")
        print(f"   teacher_contents: {len(tc)} sessions")
        if todo_count:
            print(f"\n📝 NOTE: {todo_count} [★ FILL-IN] markers need manual completion.")
            print(f"   Edit: public/data/lessons/W{week_num}.json")
        return True

    # ── Route: W1-W53 → DOCX parser ────────────────────────────────────────
    try:
        import build_lesson_plans_from_docx as bld
    except ImportError as e:
        print(f"❌ ERROR: Cannot import build_lesson_plans_from_docx: {e}")
        return False

    # Check reference DOCX exists
    if not bld.REFERENCE_DOCX.exists():
        print(f"❌ ERROR: Reference DOCX not found: {bld.REFERENCE_DOCX}")
        return False

    print(f"  Source DOCX: {bld.REFERENCE_DOCX.name}")

    # Load DOCX (cached if already loaded — re-import will re-run)
    import parse_docx_lessons as pml
    doc, paras, body_items, para_to_bi = pml.load_doc(str(bld.REFERENCE_DOCX))
    ranges, order = pml.find_week_ranges(paras)

    if week_num not in ranges:
        print(f"❌ ERROR: Week {week_num} not found in reference DOCX (found: {order})")
        return False

    wstart, wend = ranges[week_num]
    data = bld.build_week(doc, paras, body_items, para_to_bi, week_num, wstart, wend)
    bld.inject_week(week_num, data)

    tc = data.get('teacher_contents', [])
    missing_ls = [s['session'] for s in tc if not s.get('listening_script', {}).get('text')]
    if missing_ls:
        print(f"\n⚠️  WARN: Sessions {missing_ls} have no listening_script.text")
    else:
        print(f"\n✅ PHASE 6 COMPLETE — W{week_num} injected into 5 files")
        print(f"   unit_theme: {data.get('unit_theme','')}")
        print(f"   teacher_contents: {len(tc)} sessions")
...
def main():
    ...
    print("   ")
    print("7. TEACHER PANEL LESSON PLAN (Phase 6):")
    print(f"   python3 mass_produce_week.py {week_num} --phase 6")
    if week_num >= 54:
        print(f"   Source: _syllabus_v5_raw.txt (auto-generated via SyllabusParser)")
        print(f"   NOTE: Search [★ FILL-IN] in public/data/lessons/W{week_num}.json and complete manually.")
    else:
        print(f"   Source: 0. NEW_FINAL_Lesson plans_W*.docx (reference DOCX)")
    print(f"   Updates 5 files: lessonPlans.json, lessonPlans_index.json (×2),")
    print(f"           mcp-server/data/lessons/W{week_num}.json, public/data/lessons/W{week_num}.json")
    print("   ")
    print("8. COMMIT:")
    ...
    print(f"   git add public/data/lessons/W{week_num}.json mcp-server/data/lessons/W{week_num}.json")
    print(f"   git add public/data/lessonPlans.json public/data/lessonPlans_index.json mcp-server/data/lessonPlans_index.json")
    print(f'   git commit -m "Week {week_num}: {WEEK_CONFIG.get("theme_en", "Complete")}"')
    print("   ")
    print("9. DEPLOY:")
    print("   git push origin main  # Auto-deploy via Cloudflare Pages / Railway")
    print("="*60)
