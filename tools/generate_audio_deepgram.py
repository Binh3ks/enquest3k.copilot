...
# ─────────────────────────────────────────────────────────────────────────────
#  TEXT HASH CACHE — prevents stale audio from being reused
# ─────────────────────────────────────────────────────────────────────────────
# Problem: text changes but audio_url stays the same → R2 returns stale audio.
# Fix: track (r2_key → SHA256(text)), skip only if hash matches.
# If hash mismatches: force regenerate, overwrite R2 cache.

class TTSTextCache:
    """
    Manages tts_manifest.json: { r2_key → sha256(text) }
    Also stores per-entry metadata: generated_at, model.

    Usage:
        cache = TTSTextCache(manifest_path)
        stored_hash = cache.get_hash(r2_key)
        current_hash = cache.hash_text(text)
        if stored_hash != current_hash:
            # text changed → must regenerate
            cache.set_hash(r2_key, current_hash, model=model)
            cache.save()
    """
    ...
    def hash_text(self, text: str) -> str:
        """SHA-256 hash of normalized TTS text."""
        normalized = text.replace("o'clock", "o clock").replace("\n", " ").strip()
        return hashlib.sha256(normalized.encode("utf-8")).hexdigest()[:16]

    def get_hash(self, r2_key: str) -> str | None:
        entry = self.data.get(r2_key, {})
        return entry.get("text_hash") or entry.get("hash")

    def set_hash(self, r2_key: str, text_hash: str, model: str = ""):
        if r2_key not in self.data:
            self.data[r2_key] = {}
        self.data[r2_key]["text_hash"] = text_hash
        self.data[r2_key]["generated_at"] = time.strftime("%Y-%m-%d")
        if model:
            self.data[r2_key]["model"] = model

    def is_fresh(self, r2_key: str, text: str) -> bool:
        """Return True if cached hash matches current text."""
        return self.get_hash(r2_key) == self.hash_text(text)
    ...
    def stats(self) -> dict:
        return {
            ...
            "fresh": sum(1 for k in self.data if self.data[k].get("text_hash")),
        }

    def stale_entries(self) -> list[str]:
        """Return list of r2_keys that have no hash (legacy entries)."""
        return [k for k, v in self.data.items() if not v.get("text_hash")]
...
def scan_for_tasks(data_path: Path, voice_config: dict) -> list:
    ...
    def extract_read_explore(content, file_path):
        ...
        if sentences:
            ...
            text = re.sub(r"<.*?>", "", text)
            text = text.replace('\n', ' ').strip()
        # read.js uses audio_url: "/audio/weekN/read_main.mp3"
        # explore.js uses audio_url: "/audio/weekN/explore_main.mp3"
        fname = "read_main.mp3" if file_path.stem == "read" else "explore_main.mp3"
        _task(text, "narration", fname, "read_explore")

        # Extract comprehension question audio from read.js
        if file_path.stem == "read":
            # Match comprehension_questions[...{ id: N, question_en: "...", answer: [...], ... audio_url: "..." }]
            for obj in re.findall(r'\{\s*id:\s*\d+[^}]+\}', content, re.DOTALL):
                qid_m = re.search(r'id:\s*(\d+)', obj)
                audio_m = re.search(r'audio_url:\s*["\']([^"\']+)["\']', obj)
                if qid_m and audio_m:
                    qid = qid_m.group(1)
                    # Extract question text for TTS
                    qtext_m = re.search(r'question_en:\s*["\']([^"\']+)["\']', obj)
                    if qtext_m:
                        _task(qtext_m.group(1), "questions", f"read_q{qid}.mp3", "read_explore")
    ...
    def extract_mindmap(content, file_path):
        ...
        if cs:
            ...
            for i, t in enumerate(stem_list):
                # Replace ___ with comma for natural pause, not dots (which TTS reads as "uh uh")
                clean = t.replace('___', ',')
                # Remove trailing periods before adding padding
                clean = re.sub(r'\.\s*$', '', clean.strip())
                if clean:
                    # Add longer trailing silence to prevent cutoff (especially for o'clock)
...
def run_for_week_mode(
    ...
    *,
    ...
    # Text hash cache — detects when text changes so we regenerate stale audio
    cache = TTSTextCache(root / "tools" / "tts_manifest.json")
    stale_detected = 0
    ...
    for i, task in enumerate(tasks, 1):
        ...
        if not force:
            if local_file.exists() and (not check_text or cache.is_fresh(r2_key, text)):
                skipped += 1
                continue
            if not local_only and file_exists_on_r2(r2_key):
                # R2 has the file — but check text hash before skipping
                if check_text and not cache.is_fresh(r2_key, text):
                    stale_detected += 1
                    print(f"  {prefix} ⚠️  STALE (text changed) → regenerating: {filename}")
                else:
        ...
        if success:
            generated += 1
            if check_text:
                cache.set_hash(r2_key, cache.hash_text(text), model=model)
            if auto_upload:
    ...
    # Save text hash manifest after processing this week
    if check_text:
        cache.save()
        stats = cache.stats()
        print(f"  📋 Text hash cache: {stats['total_entries']} entries saved")
...
def main():
    ...
    parser.add_argument("week",
        help="Week number (1-33) or 'all' to process all weeks")
    ...
    parser.add_argument("--check-text", dest="check_text", action="store_true", default=True,
        help="Detect text changes via SHA-256 hash — regenerate stale audio (default: on)")
    parser.add_argument("--no-check-text", dest="check_text", action="store_false",
        help="Disable text-hash check — use R2 existence only (legacy mode, may use stale audio)")
    parser.add_argument("--audit-stale", action="store_true",
        help="Scan and report stale audio entries (text changed since last generation). Does NOT generate.")
    ...
    # ── Weeks ─────────────────────────────────────────────────────────────────
    if args.week == "all":
        weeks = list(range(1, 34))   # 1–33
    else:
        try:
            weeks = [int(args.week)]
        except ValueError:
            print(f"❌  Invalid week '{args.week}'. Use 1-33 or 'all'.")
            sys.exit(1)

    modes = ["advanced", "easy"] if args.mode == "all" else [args.mode]

    # ── Audit stale audio mode ─────────────────────────────────────────────────
    if args.audit_stale:
        import urllib.request
        print(f"\n{'='*60}")
        print(f"  🔍 AUDIT STALE AUDIO — Comparing text hashes vs R2")
        print(f"{'='*60}\n")
        manifest_path = Path(__file__).parent.parent / "tools" / "tts_manifest.json"
        cache = TTSTextCache(manifest_path)
        stale_count = 0
        for r2_key, entry in sorted(cache.data.items()):
            text_hash = entry.get("text_hash") or entry.get("hash")
            if not text_hash:
                continue  # legacy entry, no hash
            ...
            print(f"  {'✅' if exists else '❌'} [{generated}] {r2_key} (hash={text_hash})")
        print(f"\n  Total manifest entries: {cache.stats()['total_entries']}")
        print(f"  Note: R2 existence check above. Run without --audit-stale to regenerate stale audio.\n")
        return
