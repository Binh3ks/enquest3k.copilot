...
const MAX_DURATION = 720;  // Maximum 12 minutes - ESL kids attention span
const IDEAL_MAX = 480;     // Ideal max 8 minutes per video
...
const titleMatchesQuery = (title, query, purpose = 'TOPIC') => {
  ...
  // CRITICAL: Topic-level exclusion - prevent wrong-topic videos
  // If query is about PRESENT tense, reject PAST tense videos and vice versa
  if (queryLower.includes('present continuous') || queryLower.includes('present progressive')) {
    if (titleLower.includes('past tense') || titleLower.includes('past simple')) return false;
  }
  if (queryLower.includes('past simple') || queryLower.includes('past tense') || queryLower.includes('was were') || queryLower.includes('there was')) {
    if (titleLower.includes('present continuous') || titleLower.includes('present progressive')) return false;
    if (titleLower.match(/\bam is are\b/) && !titleLower.match(/was|were|did/)) return false;
  }
  // If query is about Places/Weekend, reject past tense or too-academic videos
  if (queryLower.includes('places') || queryLower.includes('weekend') || queryLower.includes('my town')) {
    if (titleLower.includes('long ago and now') || titleLower.includes('history')) return false;
  }
