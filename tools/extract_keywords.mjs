...
for (let week = 1; week <= MAX_WEEK; week++) {
  ...
  for (const path of paths) {
    ...
    try {
      ...
      // read.js
      if (data?.content_en) {
        extractFromContent(data.content_en);
      }
      
      // explore.js (multiple sections)
      if (data?.sections) {
        for (const section of data.sections) {
          if (section.content_en) extractFromContent(section.content_en);
          if (section.description) extractFromContent(section.description);
        }
      }
    } catch (err) {
