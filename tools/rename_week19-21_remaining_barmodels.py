...
TARGETS = []
for p in range(1, 6):
    TARGETS.append((f"barmodel_w19_adv_p{p}_v1.jpg", Path("public/images/week19")))
for p in range(1, 6):
    TARGETS.append((f"barmodel_w20_adv_p{p}_v2.jpg", Path("public/images/week20")))
for p in range(1, 6):
    TARGETS.append((f"barmodel_w21_easy_p{p}_v1.jpg", Path("public/images/week21")))

NAME_PATTERNS = [
    re.compile(r"(barmodel_w19_adv_p[1-5]_v1)_jpg", re.I),
    re.compile(r"(barmodel_w20_adv_p[1-5]_v2)_jpg", re.I),
    re.compile(r"(barmodel_w21_easy_p[1-5]_v1)_jpg", re.I),
]
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3 tools/rename_week19-21_remaining_barmodels.py "/path/to/source"')
        sys.exit(1)
    ...
    if len(named_matches) == 15:
        ...
        if len(files) != 15:
            print(f'Expected 15 files, found {len(files)}')
            for file in files:
                print(f'  {file.name}')
            sys.exit(1)
        mapping = []
    ...
    for index, (src_file, dest_file) in enumerate(mapping, 1):
        ...
        print(f'[{index:02d}/15] {src_file.name} -> {dest_file}')
