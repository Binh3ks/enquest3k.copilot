...
/**
 * Sanity-check: compare problem `type` with question semantics to catch
 * visual mismatch before generating bar model images.
 *
 * Returns null if OK, or an error string if likely wrong.
 */
function typeSanityCheck(p) {
  const q = (p.question_en || p.question || "").toLowerCase();
  const t = p.type;

  const qLast = q.replace(/[?.,!]+$/, '').split(/\s+/).slice(-3).join(' ');
  const nums = extractNumbers(q);

  // ADDITION signals: total unknown (bar = PartA + PartB)
  const isAddition = (
    q.replace(/[?.,!]+$/, '').endsWith('in total')
    || q.replace(/[?.,!]+$/, '').endsWith('in all')
    || q.replace(/[?.,!]+$/, '').endsWith('altogether')
  );

  // SUBTRACTION signals: total known, one part unknown
  const isStructuralSub = (
    /\bin \d+ trips\b/.test(q)
    || /\bin \d+ groups\b/.test(q)
    || /\bmore than\b/.test(q)
  );
  const isQuestionSub = (
    /\bleft\b/.test(qLast)
    || /\bnow\b/.test(qLast)
    || /\brunning\b/.test(qLast)
    || /\bhow many more\b/.test(q)
    || /\bhow much more\b/.test(q)
    || /\bstill need to\b/.test(q)
    || /\bneeded to finish\b/.test(q)
    || /\bgive away\b/.test(q)
    || /\bgave away\b/.test(q)
    || /\bsteps left\b/.test(q)
  );
  const isSubtraction = isStructuralSub || isQuestionSub;

  if (t === 'part_whole') {
    if (isAddition && nums.length >= 2) {
      // For addition questions, bar should show PartA + PartB = Total:? (NOT total known)
      // This is a semantic mismatch if the question asks for the total
      // but the bar model shows total as known. We WARN only.
      return `⚠️  part_whole + 'in total' question: bar model will show Total:? with two parts — CORRECT for addition`;
    }
    if (isSubtraction && nums.length >= 2) {
      // Should be SUBTRACTION type for this pattern
      return `⚠️  part_whole with subtraction signals: consider type='missing_part' for Total-known bar model`;
    }
  }

  if (t === 'missing_part' && isAddition) {
    return `⚠️  missing_part with 'in total' question: consider type='part_whole' for addition bar model`;
  }

  // For before_after: check if it should be missing_part instead
  if (t === 'before_after') {
    const findStartKw = /\bat first\b/.test(q) || /\bhad some\b/.test(q)
      || /\bcollected some\b/.test(q) || /\bate some\b/.test(q)
      || /\btake\b/.test(qLast);
    if (findStartKw) {
      return `⚠️  before_after 'find start' type: bar model will show Start:?, Result, Change — verify this matches intent`;
    }
  }
...
async function main() {
  ...
  for (const result of results) {
    ...
    for (const p of problems) {
      ...
      // Check 4: type semantics sanity check
      const sanity = typeSanityCheck(p);
      if (sanity) {
        console.warn(`   ℹ️  ${sanity}`);
      }
