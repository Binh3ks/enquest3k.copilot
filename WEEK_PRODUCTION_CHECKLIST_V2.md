...
#### ✅ 1.5 Conversation Cards (FreeTalk Tab) — BUG-W23-A

⚠️ **CRITICAL: FreeTalkTab.jsx reads `card.exchanges` directly — missing exchanges = TypeError crash**

```javascript
  conversation_cards: [
    {
      id: "string_id_no_spaces",       // STRING, not number — e.g. "art_class_painting"
      title: "Card Title",             // Shown in UI
      emoji: "🎨",                     // Single emoji
      theme: "Talking about X using Y grammar",
      difficulty: "easy",              // "easy" | "medium" | "hard"
      exchanges: [                     // REQUIRED — minimum 4, recommended 5-6
        {
          ai: "Nova's prompt text. Options: A) ... B) ...",
          options: ["Option A text", "Option B text"]
        },
        // ... 4-5 more exchanges
      ],
      completion_message: "Congratulations! You used [week grammar] correctly!"
    },
    // Card 2 (different topic, same week)
    // Card 3 (different topic, same week)
  ],
```

**Rules:**
- [ ] Exactly 3 conversation cards
- [ ] `id` is a STRING (not number 1, 2, 3) — use snake_case describing topic
- [ ] `exchanges` array has 4-6 items per card
- [ ] Each exchange has both `ai` and `options` fields
- [ ] `completion_message` references week grammar pattern
- [ ] Topics are DISTINCT (not 3 versions of same thing)
- [ ] `difficulty` matches topic complexity
- [ ] **GATE CHECK**: CHECK 39 in `code_quality_gate.sh` auto-detects legacy format

**Reference:** `src/data/weeks/week_22_real.js` lines ~380-460
...
⚠️ **AFTER creating singapore_math.js — Run bar model image generation immediately (BUG-W23-C):**
```bash
# Generate bar model images for both modes (10 files: adv p1-5, easy p1-5)
python3 generate_logiclab_barmodels.py [N]

# ⚡ IMMEDIATELY upload to R2 CDN — images DO NOT deploy automatically via git
python3 tools/upload_week_images_r2.py [N]
```
- [ ] ✅ `public/images/week[N]/barmodel_w[N]_adv_p1..p5_v1.jpg` — 5 files generated
- [ ] ✅ `public/images/week[N]/barmodel_w[N]_easy_p1..p5_v1.jpg` — 5 files generated
- [ ] ✅ R2 upload confirmed (CHECK 41 in code_quality_gate.sh will verify)

**⚠️ RULE: Bar model filenames must NOT appear in image_prompts.txt (CHECK 40)**
Bar models are generated by Python script, not by AI image generation workflow.
Image prompts = vocab images + wordpower images + cover images ONLY.
...
**Image types and how each is generated (W20+ unified folder model):**

| Image type | Count | Generated by | Uploaded to R2 by |
|---|---|---|---|
| vocab images | N (ONE set, shared adv+easy) | AI image generator (image_prompts.txt) | `upload_week_images_r2.py` |
| wordpower images | 6 (ONE set, shared adv+easy) | AI image generator (image_prompts.txt) | `upload_week_images_r2.py` |
| read cover | 1 | AI image generator (image_prompts.txt) | `upload_week_images_r2.py` |
| explore cover | 1 | AI image generator (image_prompts.txt) | `upload_week_images_r2.py` |
| bar models | 10 (adv p1-5 + easy p1-5) | `python3 generate_logiclab_barmodels.py [N]` | `upload_week_images_r2.py` |

**TOTAL: N+18 images per week** (all in ONE folder `public/images/weekNN/` — NO `weekNN_easy` folder)

⚠️ **WEEK 20+ = ONE folder `weekNN/` for BOTH modes. `weekNN_easy` must NOT be created.**

**Step 1 — Create image prompts file (vocab/wordpower/covers only):**
```bash
# Location: Production_FINAL/IMAGE PROMPTS/week_[N]_image_prompts.txt
# Must contain exactly 21 entries: vocab (N) + wordpower (6) + covers (2)
# DO NOT include barmodel_ entries
```

**Step 2 — Generate bar model images (W16+):**
```bash
# Already run in STEP 3.11 — verify files exist
ls public/images/week[N]/barmodel_w[N]_*.jpg | wc -l  # expect 10
```

**Step 3 — Upload ALL images to R2 CDN:**
```bash
# This uploads both AI-generated images AND bar models
python3 tools/upload_week_images_r2.py [N]
# ⚡ MANDATORY — images committed to git do NOT auto-deploy to R2
```

**Checklist:**
- [ ] `Production_FINAL/IMAGE PROMPTS/week_[N]_image_prompts.txt` created (21 entries, no barmodel_ lines)
- [ ] 10 vocab images generated by AI (clear, kid-friendly)
- [ ] 3 word_power images generated by AI
- [ ] 1 read cover + 1 explore cover generated by AI
- [ ] 10 bar model images exist in `public/images/week[N]/`
- [ ] ✅ `python3 tools/upload_week_images_r2.py [N]` run and all uploads confirmed
- [ ] **GATE CHECK**: `bash tools/code_quality_gate.sh [N]` CHECK 40 + CHECK 41 pass
