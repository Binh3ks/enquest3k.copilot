#!/usr/bin/env python3
"""
Batch Generator for Week 37 (v6 Rich Context Standard)
...
- Prompt: 100% RAW string prompts from week_37_image_prompts.txt (v6 Rich Context)
- Output: 1024x1024 High Quality JPEGs overwritten to public/images/week37/
"""
...
def parse_w37_v6_prompts():
    filepath = "Production_FINAL_DEPRECATED/IMAGE PROMPTS/week_37_image_prompts.txt"
    items = []
    with open(filepath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if m:
                filename = m.group(1)
                prompt = m.group(2)
                full_path = os.path.join("public/images/week37", filename)
        ...
        if resp.status_code == 200:
            ...
            if items and "b64_json" in items[0]:
                ...
                if len(img_bytes) > 20000:
                    ...
                    return True, len(img_bytes), "Together.ai (FLUX.1-schnell)"
            ...
            if cands:
                ...
                for p in parts:
                    if "inlineData" in p:
                        ...
                        return True, len(raw_bytes), "Google AI Studio (Fallback)"
    except Exception as e:
...
def main():
    ...
    items = parse_w37_v6_prompts()
    print(f"🚀 Generating all {len(items)} Week 37 images (v6 Rich Context) using Frozen Pipeline Together.ai FLUX.1-schnell...")
    ...
    for idx, (filepath, prompt) in enumerate(items, start=1):
        ...
        ok, size, provider = generate_together_ai(together_key, prompt, filepath)
        if not ok and gemini_key:
            ok, size, provider = generate_google_fallback(gemini_key, prompt, filepath)
        item_elapsed = time.time() - item_start
        if ok:
            results.append((os.path.basename(filepath), size, provider, item_elapsed))
        time.sleep(0.5)
        
    total_elapsed = time.time() - start_time
    print("\n================ RENDER REPORT W37 (v6 Rich Context) ================")
    for fname, size, provider, elapsed in results:
        print(f"  - {fname:<25}: {size:7d} B ({size/1024:6.1f} KB) | {elapsed:4.1f}s | {provider}")
    print(f"\n🎉 Total: {len(results)}/{len(items)} rendered successfully in {total_elapsed:.1f} seconds.")
