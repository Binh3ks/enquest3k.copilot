...
def download_image(filepath, prompt, seed):
...
def main():
    print(f"🚀 Re-downloading {len(tasks)} ultra-precise 3D Pixar card images...")
    success = 0
    for idx, (filepath, prompt) in enumerate(tasks):
        print(f"[{idx+1}/{len(tasks)}] Downloading {filepath}...")
        # Use fixed unique seed based on index so it is deterministic and high quality
        seed = 9999 + idx * 7
        if download_image(filepath, prompt, seed):
            success += 1
        time.sleep(2.5)
    print(f"🎉 Complete! Downloaded {success}/{len(tasks)} ultra-precise 3D Pixar cards.")
