...
async function main() {
  console.log('================================================================');
  console.log('🧪 RUNNING ON PRODUCTION/STAGING ORIGIN TAB (HTTPS MIC PERMISSION)');
  console.log('================================================================');
  ...
  let targetTab = tabs.find(t => t.id === '1530' || t.url.includes('bkbacademy.vn'));
  ...
  `);
  console.log('Record Trigger Click:', clickRes);

  console.log('⏳ RECORDING FOR 5.5s WHILE SPEAKER PLAYS MODEL AUDIO (Student remains 100% silent)...');
  ...
  console.log('\n================================================================');
  console.log('📊 DOM EVALUATION ON TAB 1530:');
  console.log('================================================================');
  console.log(JSON.stringify(evalResult, null, 2));

  const shotFile = 'scripts/android_no_headphone_tab1530_evidence.png';
  execSync(`adb exec-out screencap -p > ${shotFile}`);
  console.log(`📸 Screenshot saved: ${shotFile}`);

  fs.writeFileSync('scripts/android_no_headphone_tab1530_report.json', JSON.stringify({
    timestamp: new Date().toISOString(),
    device: 'Samsung Galaxy S23 (SM-S911B)',
    test: 'Speaker Loopback Test without Headphones on HTTPS Origin',
    evalResult,
    logs: capturedLogs
  }, null, 2));

  console.log('\n🎉 TAB 1530 TEST COMPLETED!');
  process.exit(0);
}
