...
def fetch_single(filepath, prompt, seed):
...
def main():
    pending = [item for item in flashcards if not is_valid(item[0])]
    print(f"🚀 Processing {len(pending)} / {len(flashcards)} remaining Pixar 3D Flashcards sequentially...")
    
    for idx, (filepath, prompt, seed) in enumerate(pending):
        print(f"[{idx+1}/{len(pending)}] Fetching {filepath}...")
        fetch_single(filepath, prompt, seed)
        time.sleep(3.5)
        
    print("🎉 Sequential Pixar 3D Flashcards fetch complete!")
