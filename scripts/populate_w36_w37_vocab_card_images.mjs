...
// Generate clean vector card illustrations for W36 and W37 vocab words
const w36Words = [
...
function generateSVGCard(item) {
  ...
  <rect x="20" y="20" width="360" height="360" rx="24" fill="#ffffff" fill-opacity="0.9"/>
  ...
  <text x="200" y="320" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" font-size="26" font-weight="900" fill="${item.color}" text-anchor="middle" letter-spacing="2">${item.text}</text>
</svg>`;
}
...
function processWeek(weekNum, list) {
  ...
  for (const item of list) {
    ...
    fs.writeFileSync(jpgPath, svgContent, 'utf-8'); // SVG saved as asset
  }
  console.log(`✅ Generated 20 vocab card vector illustrations for Week ${weekNum}`);
}

processWeek(36, w36Words);
processWeek(37, w37Words);
