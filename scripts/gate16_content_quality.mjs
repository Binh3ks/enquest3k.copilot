...
async function runAudit() {
  ...
  if (!clil) {
    ...
    const factUnits = sentences.filter(s => {
      ...
      const hasSubject = /\b(squirrel|squirrels|jay|jays|bee|bees|bird|birds|lion|mouse|animal|animals|plants|trees|flowers)\b/.test(lower);
      const hasAction = /\b(eat|bury|hide|fly|drink|carry|grow|help|travel|cooperate|start|call|stays)\b/.test(lower);
      const hasOutcome = /\b(ground|spring|trees|plants|nectar|pollen|helpers|green|strong|food|homes|seeds)\b/.test(lower);
      return hasSubject && hasAction && hasOutcome;
    });
  ...
  if (!wc || !wc.setting_time || !wc.action_manner || !wc.problem_event || !wc.solution_outcome) {
    ...
    const banned = ["corridor", "jake", "friction", "nurse", "walking carefully", "moving forward", "during the journey", "stopped immediately", "needed urgent help", "was patrolling"];
  ...
  // 3. Check science_report pills overlap with CLIL content
  ...
  const lh = lhMod.listeningHub || lhMod.default || lhMod;
  ...
  if (lhMath.length !== 5 || smMath.length !== 5) {
    ...
    for (let i = 0; i < 5; i++) {
      const lhText = (lhMath[i]?.problem_en || '').trim();
      const smText = (smMath[i]?.problemText || '').trim();
      if (lhText !== smText) {
