            ...
            with urllib.request.urlopen(req, timeout=35) as resp:
                ...
                if len(data) > 15000 and data.startswith(b'\xff\xd8'):
                    with open(filepath, 'wb') as f:
                        f.write(data)
                    return True, filepath, f"Saved ({len(data)} B)"
...
def main():
    print(f"🚀 Running Fast Parallel Pixar 3D Flashcards Fetch ({len(flashcards)} items)...")
    with ThreadPoolExecutor(max_workers=3) as executor:
        results = executor.map(fetch_single, flashcards)
        for ok, fp, msg in results:
            status = "✅" if ok else "❌"
            print(f"{status} {fp} -> {msg}")
            time.sleep(1.2)
    print("🎉 Fast Parallel Pixar 3D Flashcards complete!")
