...
## ⚠️ CRITICAL: SOURCE FILES YOU MUST READ FIRST

**BEFORE generating ANY week content (Week 5-156), you MUST read these two master files:**

### 1️⃣ **Syllabus File**: `1. NEW-FINAL_Khung CT_SYLLABUS_3yrs copy.txt`
- Contains the 156-week curriculum structure
- Lists all topics, vocabulary, and learning objectives
- **Required for**: Understanding what each week teaches

### 2️⃣ **Blueprint File**: `2. ENGQUEST APP MASTER BLUEPRINT-FINAL copy.txt`
- Contains the Dual-Mode Strategy details
- Shows EXACT differentiation between Easy vs Advanced modes
- Lists specific vocabulary for EACH week in EACH mode
- **Required for**: Ensuring Easy ≠ Advanced (different themes, contexts, vocabularies)

### ⚠️ Why This Matters - Week 5 Example

**WRONG** (what AI generated before reading files):
- Easy W5 vocab: bedroom, kitchen, bathroom, living_room, bed, chair, table, house, mystery, explore
- Advanced W5 vocab: **SAME** (bedroom, kitchen, bathroom...) ❌ IDENTICAL

**CORRECT** (after reading Syllabus + Blueprint):
- Easy W5 vocab: bedroom, kitchen, bathroom, living_room, bed, chair, table, house, mystery, explore
  - Theme: "Room Tour" - exploring different rooms in a house
- Advanced W5 vocab: sofa, fridge, table, chair, lamp, mirror, cabinet, shelf, rug, upstairs
  - Theme: "Smart Home" - furniture, interior design, appliances ✅ DIFFERENT

### Workflow Rule
1. **Read Syllabus** → Understand base curriculum
2. **Read Blueprint** → Understand Dual-Mode differentiation
3. **Run `generate_spec.cjs`** → Creates vocabulary list for that week
4. **Validate** → Ensure Easy ≠ Advanced by comparing to Blueprint examples
5. **Generate Assets** → Create audio/images using `generate_complete_audio.js`
...
export default {
  ...
  freetalk_knowledge: {
    ...
    // Knowledge base for general chat and questions
    knowledge_base: [
      "Daily routines: wake up, brush teeth, eat breakfast, go to school",
      "Time expressions: in the morning, at night, at 7 AM",
      "Grammar: Present Simple for routines (I wake up, I go to school)",
      "Sequence words: first, then, after that, finally",
      "Vietnamese kids often wake up early for school",
      "Breakfast is important for energy",
      "Good habits: brush teeth twice a day, wash hands before eating"
    ],
    
    // Example opening questions for chat mode
    ...
    // ✅ FREE TALK 3.0: Starter prompts (Fixed buttons - REQUIRED!)
    starter_prompts: [
...
- ✅ `starter_prompts` array with 4 buttons (Game, Translate, Roleplay, Ask) - REQUIRED for Free Talk 3.0!
...
## 🎮 FREE TALK 3.0 ARCHITECTURE

### Overview

**Free Talk 3.0** is a modular chat system with 5 distinct modes:
1. **GAME MODE**: Structured play (I Spy, Word Chain, Emoji Mixer)
2. **ROLEPLAY MODE**: Character immersion (Pizza Chef, Pet Doctor, Toy Shop)
3. **TRANSLATION MODE**: Bilingual dictionary (English ↔ Vietnamese)
4. **KNOWLEDGE MODE**: Encyclopedia for kids (Why/How/What questions)
5. **CHAT MODE**: Natural conversation fallback

### Code Architecture

**File Location**: `src/services/ai_tutor/freeTalkModes.js` (325 lines)

**Integration**: Separated from main tutorPrompts.js (1242 lines) for cleaner organization

**Function**: `buildFreeTalkPrompt(mode, context, userMessage, options)`

### Mode Detection Logic

```javascript
// Priority order (highest to lowest):
1. Knowledge Mode: "ask me anything", "why?", "how?", "what is?"
2. Game Mode: "start_game:I Spy", isInGame = true
3. Roleplay Mode: "start_roleplay:Pizza Chef", isInRoleplay = true
4. Translation Mode: "translate", "là gì", "how to say"
5. Chat Mode: Default fallback
```
...
**Supported Games**:
- **I Spy**: AI describes object (color, shape), kid guesses
- **Word Chain**: Say word, kid says word starting with last letter
- **Emoji Mixer**: Show 2 emojis, kid guesses combined word

**Trigger**: 
- User clicks button: `start_game:I Spy`
- Ongoing game detected: `isInGame = true`

**Critical Rules**:
- ✅ Play exactly 10 rounds per game
- ✅ Show "Round [X]/10" in every response
- ✅ Give hints if user says "gợi ý", "hint", "I don't know"
- ✅ Continue game if user says "tiếp tục", "next"
- ⛔ NEVER leave game mode unless user says "stop" or "goodbye"
- ⛔ NEVER offer translation during game

**Example Flow**:
```
AI: "Round 1/10: I spy something blue and round! 🔵 What is it?"
User: "Ball?"
AI: "Yes! Great! 🎉 Round 2/10: I spy something yellow and bright! ☀️"
User: "gợi ý"
AI: "Hint: It's in the sky! You see it every day! ☀️"
User: "Sun?"
AI: "Perfect! 🎉 Round 3/10: I spy something..."
```

**Implementation**:
```javascript
if (isInGame && !startingNewGame) {
  return `SYSTEM_MODE: GAME_MASTER_ONGOING
  ROLE: Ms. Nova continuing the game
  RULES:
  1. IF user asks for hint -> Give helpful hint
  2. IF user says "tiếp tục"/"next" -> Start NEXT round
  3. Play exactly 10 rounds. Show "Round [X]/10"
  4. After Round 10/10: "Game Over! Great job! 🎉"`;
}
```
...
**Supported Scenarios**:
- **Pizza Chef**: Kid is chef, AI is hungry customer 🍕
- **Pet Doctor**: Kid is doctor, AI has sick cat 😿
- **Toy Shop**: Kid is seller, AI wants robot 🤖

**Trigger**:
- User clicks button: `start_roleplay:Pizza Chef`
- Ongoing roleplay detected: `isInRoleplay = true`

**Critical Rules**:
- ✅ Stay in character 100% - NEVER break roleplay
- ✅ Play exactly 10 exchanges per roleplay
- ✅ Show "Turn [X]/10" in every response
- ✅ Use simple English (A0-A1 level)
- ⛔ NEVER ask about real life
- ⛔ NEVER offer translation during roleplay
- ⛔ NEVER leave roleplay unless user says "stop" or "goodbye"

**Example Flow**:
```
AI: "Turn 1/10: Hello! I am hungry! 🍕 I want pizza with cheese!"
User: "OK! What size?"
AI: "Turn 2/10: Big pizza please! 🍕 Do you have pepperoni?"
User: "Yes! Here you go!"
AI: "Turn 3/10: Thank you! How much is it? 💰"
```

**Implementation**:
```javascript
if (isInRoleplay && !startingNewGame) {
  return `SYSTEM_MODE: ROLEPLAY_ACTOR_ONGOING
  ROLE: Ms. Nova continuing roleplay
  RULES:
  1. Stay in character 100%
  2. React naturally to user's response in character
  3. Play exactly 10 exchanges. Show "Turn [X]/10"
  4. After Turn 10/10: Say goodbye in character`;
}
```
...
**Purpose**: Bilingual dictionary for Vietnamese kids learning English

**Trigger**:
- User clicks button: "Translate this for me..."
- User types: "translate", "nghĩa là gì", "là gì", "how to say"
- AI asks: "Do you want to know the meaning?" → User: "yes"

**Critical Rules**:
- ✅ ALWAYS include Vietnamese translation for English words
- ✅ Format: `[English] = [Vietnamese]! [emoji] Spell it.`
- ✅ Follow up with related question: "What other [category] do you know?"
- ⛔ NEVER just define in English only
- ⛔ NEVER say "You like [word]!" or start new topic
- ⛔ NEVER deflect translation request

**Example Flow**:
```
User: "tiger"
AI: "Tiger = hổ! 🐯 T-I-G-E-R. What other wild animals do you know?"

User: "con mèo"
AI: "Con mèo = CAT! 🐱 C-A-T. What other pets do you know?"

User: "translate"
AI: "What word do you want to learn? 😊"
User: "deer"
AI: "Deer = hươu hoặc nai! 🦌 D-E-E-R. What other animals do you know?"
```

**Implementation**:
```javascript
if (isTranslationRequest) {
  return `SYSTEM_MODE: BILINGUAL_DICTIONARY
  ROLE: Ms. Nova as bilingual translator
  RULES:
  1. IF English word -> Translate to Vietnamese!
     Example: "Deer = hươu hoặc nai! 🦌 D-E-E-R"
  2. IF Vietnamese word -> Translate to English!
     Example: "Con mèo = CAT! 🐱 C-A-T"
  3. ALWAYS include both languages
...
**Purpose**: Encyclopedia for kids - answer factual questions

**Trigger**:
- User clicks button: "I have a question! ❓"
- User asks: "Why?", "How?", "What is?" questions
- Explicit: "ask me anything"

**Critical Rules**:
- ✅ Answer factually with REAL knowledge (numbers, facts)
- ✅ Use simple words (A0-A1 level)
- ✅ Add emojis for friendliness
- ✅ If too complex (Physics/Politics): "That is a big question! 🌍 Ask your parents!"
- ⛔ NEVER deflect with "What do you like?"
- ⛔ NEVER start a game unless asked
- ⛔ NEVER give vague answers like "Very big" without specifics

**Example Flow**:
```
User: "How big is a whale?"
AI: "A whale is 20-30 meters long! Very big! 🐳 That's like 5 cars!"

User: "Why do we sleep?"
AI: "We sleep to rest our body and brain! 😴 It helps us grow strong!"

User: "What is gravity?"
AI: "That is a big question! 🌍 Ask your parents or teacher! They can explain better!"
```

**Implementation**:
```javascript
if (isKnowledgeRequest) {
  return `SYSTEM_MODE: ENCYCLOPEDIA_FOR_KIDS
  ROLE: Ms. Nova (Knowledgeable teacher)
  USER QUESTION: "${userMessage}"
  RULES:
  1. ANSWER factually but SIMPLY (A0-A1)
  2. IF too hard -> "Ask your parents!"
  3. IF simple -> Explain with numbers/facts
...
**Purpose**: Natural conversation fallback when no specific mode triggered

**Critical Rules**:
- ✅ Respond naturally to what user said
- ✅ If user says single English word → Offer to translate
- ✅ If user asks factual question → Give real knowledge
- ✅ Keep responses SHORT (under 35 words)
- ⛔ NEVER ask "What makes you happy?" or "How are you feeling?"
- ⛔ NEVER force into other modes

**Example Flow**:
```
User: "Hello!"
AI: "Hello! I am Ms. Nova 🌟. Click a button below to Play, Roleplay or Chat! 👇"

User: "tiger"
AI: "Wow! A tiger! 🐯 Do you want to know the meaning of 'tiger' in Vietnamese?"

User: "I like pizza"
AI: "Pizza is delicious! 🍕 Do you like cheese pizza or pepperoni pizza?"
```

**Implementation**:
```javascript
return `SYSTEM_MODE: CHAT_COMPANION
ROLE: Ms. Nova - friendly teacher
RULES:
1. IF first message -> Greet and show buttons
2. IF single English word -> Offer translation
3. IF factual question -> Give real knowledge
4. Otherwise -> Respond naturally
5. Keep SHORT (under 35 words)`;
```

---

### Free Talk 3.0 UI Buttons

**Location**: FreeTalkTab.jsx

**Button Array** (from `starter_prompts`):
```javascript
const buttons = [
  { 
    ...
    type: "game",
    action: "start_game:I Spy" // or Word Chain, Emoji Mixer
  },
  { 
    text_en: "Translate this for me... 📖", 
    text_vi: "Dịch giúp con câu/chữ này...", 
    type: "help",
    action: "translate" // Opens translation mode
  },
  { 
    text_en: "Let's do roleplay! 🎭", 
    text_vi: "Chơi nhập vai đi cô!", 
    type: "roleplay",
    action: "start_roleplay:Pizza Chef" // or Pet Doctor, Toy Shop
  },
  { 
    text_en: "I have a question! ❓", 
    text_vi: "Con có câu hỏi!", 
    type: "ask_anything",
    action: "ask me anything" // Opens knowledge mode
  }
];
```

**Button Behavior**:
1. Buttons appear at conversation start
2. Clicking button sends action string to `buildFreeTalkPrompt()`
3. Mode detection logic triggers appropriate prompt
4. User can switch modes by clicking different buttons

---

### AI Tutor Integration

**freetalk_knowledge Schema** (Week 4+ VARIANT):
...
freetalk_knowledge: {
  ...
  // Knowledge base for Chat & Knowledge modes
  knowledge_base: [
    ...
    // ... more facts for this week's theme
  ],
  
  // Example questions for Chat mode
  example_opening_questions: [
  ...
  // ✅ REQUIRED: Starter prompts for Free Talk 3.0 UI
  starter_prompts: [
    { text_en: "I want to play games! 🎮", text_vi: "Tôi muốn chơi game!", type: "game" },
    { text_en: "Translate this for me... 📖", text_vi: "Dịch giúp con câu/chữ này...", type: "help" },
    { text_en: "Let's do roleplay! 🎭", text_vi: "Chơi nhập vai đi cô!", type: "roleplay" },
...
**Critical**: `starter_prompts` array is REQUIRED in every week's AI Tutor file for Free Talk 3.0 to work!

---

### Mass Production Checklist for Free Talk 3.0

When generating new weeks, ensure:

- [ ] `freetalk_knowledge` object present (NOT `free_talk_knowledge`)
- [ ] `knowledge_base` array with 8-12 facts about week's theme
- [ ] `example_opening_questions` array with 6-8 questions
- [ ] **`starter_prompts` array with exactly 4 buttons** (Game, Translate, Roleplay, Ask)
- [ ] Button texts match Week 4 format exactly
- [ ] Each button has `text_en`, `text_vi`, and `type` fields

**Example for Week 5 (Daily Routines)**:
...
- ✅ Uses ack_variants (NOT ack_options)
- ✅ Uses freetalk_knowledge (NOT free_talk_knowledge)
- ✅ freetalk_knowledge has starter_prompts array (4 buttons for Free Talk 3.0)
...
### Mistake 11: Missing starter_prompts for Free Talk 3.0
❌ **Wrong**:
```javascript
// freetalk_knowledge WITHOUT starter_prompts array
freetalk_knowledge: {
  week_number: 5,
  week_title: "Daily Routines",
  knowledge_base: [...],
  example_opening_questions: [...]
  // ❌ Missing starter_prompts! Free Talk 3.0 won't work!
}
```
...
- v1.0 (Jan 19, 2026): Initial complete context document
  ...
  - Includes Free Talk 3.0 architecture (5 modes)
  - Includes 11 common mistakes
