...
export function buildFreeTalkPrompt(mode, context, userMessage, options = {}) {
  ...
  if (mode === 'translation_help') {
    ...
    if (isJustAWord) {
      ...
      ROLE: Nova — bilingual dictionary for Vietnamese kids.
...
NEVER respond in Vietnamese. ALWAYS use English.
    ...
    if (isKnowledgeRequest) {
      ...
      ROLE: Nova (Knowledgeable, Friendly teacher for kids A0-A1).
    ...
    if (isTranslationRequest) {
      ...
      if (lowerAI.includes("do you want to know the meaning") && (lowerUser === "yes" || lowerUser === "có" || lowerUser === "ok")) {
        ...
        ROLE: Nova translating the word user asked about.
      ...
      ROLE: Nova as bilingual translator for Vietnamese kids (A0-A1).
    ...
    // Xử lý chat thông thường — Nova dẫn dắt HS từ câu chuyện bài đọc sang chia sẻ thực tế của HS
    const weekTitle = context?.weekTitle || '';
    const storyCharacter = context?.storyCharacter || '';
    const storyBridgeHint = storyCharacter && storyCharacter !== 'Nova'
      ? `Opening strategy: reference "${storyCharacter}" from the reading, then ask about the student's real experience.`
      : weekTitle
        ? `Opening strategy: reference this week's topic "${weekTitle}" to start conversation.`
        : '';
    return `
    SYSTEM_MODE: CHAT_COMPANION
    ROLE: Nova - friendly English teacher for kids (A0-A1).
    TOPIC: ${topic}
    ...
    INSTRUCTIONS:
    1. IF this is the first message (user says "[SYSTEM: Start conversation]"), use the opening strategy above to start naturally.
    2. IF user says a single English word (e.g. "tiger"), offer to translate:
       "Wow! A tiger! 🐯 Do you want to know the meaning?"
    3. IF user asks factual question: give REAL knowledge with numbers/facts (under 35 words).
