...
// Inlined from turnManager to avoid circular import
function canonicalizeQuestion(question) {
  if (!question) return '';
  let normalized = question.toLowerCase().trim().replace(/[?.!,]/g, '');
  const patterns = [
  ...
  return normalized.split(/\s+/).slice(0, 5).join(' ') || normalized;
}
...
export function guardResponseObject(responseObj, context = {}, maxWords = 15) {
  ...
  // 🎯 ENFORCE NOVA STRUCTURE - Support both old and new formats
  ...
  console.log('🎯️ Enforced Nova structure:', {
    ack: ack || '(none)',
    recast: recast || '(none)',
    bridge: bridge || '(none)',
    question: question,
