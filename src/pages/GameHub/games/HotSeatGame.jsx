...
import personalBank from '../../../data/personal_questions_bank.js';
...
/** Pick the personal question pool based on week number */
function getPersonalPool(weekNumber) {
  if (weekNumber <= 10) return personalBank.level1;
  if (weekNumber <= 20) return [...personalBank.level1, ...personalBank.level2];
  return [...personalBank.level2, ...personalBank.level3];
}

// ─── Constants ───────────────────────────────────────────────────────────────

const QS_PER_GAME = 10;
const TIMER_SECS  = 15;
const MAX_LIVES   = 3;
...
function truncateStarter(starter) {
  // Show at least 3 chars of first word + ellipsis so wh-words are distinguishable
...
export default function HotSeatGame({ weekNumber = 1, learningMode = 'advanced', onGameComplete }) {
  ...
  useEffect(() => {
    ...
    const mapQ = q => {
      ...
      return {
        ...
        hint_en:        q.hint_en || '',
        isPersonal:     q._personal || false,
      };
    };

    // ── Build personal questions for this week ─────────────────────────────
    const personalPool = shuffle(getPersonalPool(weekNumber))
      .slice(0, QS_PER_GAME)           // cap to game size
      .map(q => ({ ...q, _personal: true }));
    const personalMapped = personalPool.map(mapQ)
      .filter(q => q.correctStarter);  // only include questions with valid chip

    const finalize = (readingMapped, title) => {
      // Sort reading: prefer questions with clue_statement + starter
      const great    = readingMapped.filter(q => q.hasStatement && q.correctStarter);
      const ok       = readingMapped.filter(q => !q.hasStatement && q.correctStarter);
      const fallback = readingMapped.filter(q => !q.correctStarter);
      const reading  = shuffle([...great, ...ok, ...fallback]);

      // Mix: up to ½ from reading, rest from personal, total QS_PER_GAME
      const readingSlot  = Math.ceil(QS_PER_GAME / 2);
      const personalSlot = QS_PER_GAME - Math.min(readingSlot, reading.length);
      const combined = shuffle([
        ...reading.slice(0, readingSlot),
        ...personalMapped.slice(0, personalSlot),
      ]).slice(0, QS_PER_GAME);
    ...
    if (!loader) {
      // No story for this week — use personal questions only
      setCards(personalMapped.slice(0, QS_PER_GAME));
      setStoryTitle('');
      setLoading(false);
      return;
    }
    ...
    loader().then(m => {
      ...
      const mapped = qs.map(mapQ);
      finalize(mapped, data?.title || '');
    }).catch(() => {
      // Story failed to load — fall back to personal only
      setCards(personalMapped.slice(0, QS_PER_GAME));
      setStoryTitle('');
      setLoading(false);
    });
  ...
  const ClueCard = ({ showHint = false }) => (
    <div className={`bg-white rounded-2xl shadow-lg p-6 mb-4 text-center ${revealed ? 'border-2 border-orange-300' : ''}`}>
      {/* Personal badge */}
      {card?.isPersonal && (
        <div className="inline-block bg-purple-100 text-purple-700 text-xs font-bold px-3 py-1 rounded-full mb-3 uppercase tracking-wide">
          👤 Talk About You
        </div>
      )}
      {/* Statement */}
          ...
          <div className="flex items-center justify-between mb-1">
            <div>
              ...
              <p className="text-pink-200 text-sm">
                {card?.isPersonal ? '👤 Talk About You' : storyTitle ? `"${storyTitle}"` : 'Ask the right question'}
              </p>
            </div>
