...
export default function TaskScreen({ weekData, weekId: propWeekId }) {
  ...
  return (
    <div className="ts-container relative">
      ...
      {/* Chronicles FAB — floating button bottom-right */}
      {routing?.zone !== 'boss' && (
...
// ─── Chronicles FAB ───────────────────────────────────────────────────────────
// Floating action button → /week/:id/chronicles
// Visible on all quest screens for W17+ (full 15-quest weeks).

function ChroniclesFAB({ weekId }) {
  const navigate = useNavigate();
  ...
  return (
    <button
      ...
      onClick={handleClick}
      title={`The Lexio Chronicles — ${weeklyPP} PP earned`}
      style={{
        position: 'fixed',
        bottom: 24,
        right: 24,
        ...
        boxShadow: hasBossAccess
          ? '0 4px 20px rgba(245,158,11,0.55), 0 0 0 3px rgba(245,158,11,0.2)'
    ...
    >
      {/* Embedded keyframe via style tag — avoids CSS import coupling */}
      <style>{`
        @keyframes fabPulse {
