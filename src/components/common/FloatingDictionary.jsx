...
const FloatingDictionary = () => {
    ...
    fetch('/dictionary.json')
      ...
      .then(data => {
          ...
          const dict = Array.isArray(data)
            ? Object.fromEntries(data.map(entry => [normalizeLookupKey(entry.word), entry]))
            : data;
  ...
  useEffect(() => {
    const handleMouseUp = () => {
      ...
      if (selectedText && selectedText.length > 1 && selectedText.length < 50 && selectedText.split(/\s+/).filter(Boolean).length < 6) {
        // Làm sạch từ (bỏ dấu câu)
        const cleanWord = normalizeLookupKey(selectedText);
        ...
        setSelection({ 
            word: selectedText,
            cleanWord: cleanWord,
            meaning: definition?.meaning || null,
            pronounce: definition?.pronounce || null,
            definition_en: definition?.definition_en || null,
        });
      }
    };
  ...
  // Link tra cứu ngoài
  const encodedQuery = encodeURIComponent(selection.cleanWord);
  const cambridgeUrl = `https://dictionary.cambridge.org/dictionary/english-vietnamese/${encodedQuery}`;
  const labanUrl = `https://dict.laban.vn/find?type=1&query=${encodedQuery}`;
    ...
    >
      ...
      <div className="flex justify-between items-start">
        <div className="flex-1 mr-2">
            ...
            {selection.pronounce && (
              <p className="text-xs text-slate-400 font-mono mt-0.5">{selection.pronounce}</p>
            )}
            {selection.meaning ? (
              <p className="text-base font-bold text-slate-800 mt-1 leading-snug">{selection.meaning}</p>
            ) : (
              <p className="text-sm text-slate-400 italic mt-1">Tra cứu thêm bên dưới…</p>
            )}
            {selection.definition_en && (
              <p className="text-xs text-slate-500 italic mt-1 leading-relaxed">
                {selection.definition_en.split(' ').slice(0, 10).join(' ')}{selection.definition_en.split(' ').length > 10 ? '…' : ''}
              </p>
            )}
        </div>
