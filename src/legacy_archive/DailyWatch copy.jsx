...
// --- COMPONENT CON: VideoItem (MEMOIZED) ---
const VideoItem = memo(({ video, percent, onClick }) => {
  ...
  return (
    ...
    >
      <div className="aspect-video bg-slate-200 relative">
         <img src={video.thumb || `https://img.youtube.com/vi/${video.videoId}/mqdefault.jpg`} alt={video.title} className="w-full h-full object-cover" />
  ...
  return prev.video.videoId === next.video.videoId && prev.percent === next.percent;
});

// --- MAIN COMPONENT ---
const DailyWatch = ({ data, onReportProgress }) => {
  // FIX: Dùng State thay vì useMemo cho activeVideo để tránh re-render player khi data thay đổi
    ...
    const handler = setTimeout(() => {
      if (data && data.videos && onReportProgress) {
        ...
        data.videos.forEach(v => {
           const percent = getPercent(v.id, v.sim_duration || 120);
  ...
  const startTimer = (videoId, simDuration) => {
    if (timerRef.current) clearInterval(timerRef.current);
    timerRef.current = setInterval(() => {
      setWatchData(prev => {
        const currentSeconds = prev[videoId] || 0;
        if (currentSeconds >= simDuration * 1.5) return prev; // Cap at 150%
        return { ...prev, [videoId]: currentSeconds + 1 };
      });
    }, 1000);
  };
  ...
  // PLAYER LOGIC: Chỉ chạy khi activeVideo thay đổi (User click)
  useEffect(() => {
          ...
          playerRef.current = new window.YT.Player('yt-player-frame', {
            ...
            events: {
              ...
              'onStateChange': (event) => {
                ...
                if (event.data === 1) startTimer(activeVideo.id, activeVideo.sim_duration || 120);
                else if (event.data === 0) {
                   stopTimer();
                   setWatchData(prev => ({ ...prev, [activeVideo.id]: activeVideo.sim_duration || 120 }));
                } else stopTimer();
              },
              'onError': () => {
                 stopTimer();
                 setErrorMsg("Video không khả dụng.");
              }
            }
          });
        } catch (err) { setErrorMsg("Lỗi player."); }
      }
    };
  ...
  }, [activeVideo]); // Dependency là activeVideo state object, không phụ thuộc data ngoài

  const handleOpenModal = (video) => {
    setErrorMsg(null);
    setPlayerState(-1);
    setActiveVideo(video); // Set state trực tiếp, cố định video đang xem
  };
    ...
    <div className="pb-24">
      ...
      <div className="bg-rose-100 p-4 rounded-xl border border-rose-200 mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-600 font-medium">Lưu ý: Tua video sẽ không được tính giờ!</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        {data.videos.map((video) => (
          <VideoItem 
            ...
            video={video} 
            percent={getPercent(video.id, video.sim_duration || 120)} 
                ...
                {!errorMsg && (
                   getPercent(activeVideo.id, activeVideo.sim_duration || 120) >= 90 ? (
                       ...
                       <div className="inline-flex items-center justify-center gap-2 text-green-400 font-mono bg-green-900/30 px-4 py-1 rounded-full border border-green-800 animate-pulse">
                         <Loader2 className="w-4 h-4 animate-spin" /> Recording... {getPercent(activeVideo.id, activeVideo.sim_duration || 120)}%
                       </div>
                   ) : (
                       <div className="inline-flex items-center justify-center gap-2 text-amber-400 font-mono bg-amber-900/30 px-4 py-1 rounded-full border border-amber-800">
                         <PauseCircle className="w-4 h-4" /> Paused: {getPercent(activeVideo.id, activeVideo.sim_duration || 120)}%
                       </div>
                   )
                )}
              </div>
