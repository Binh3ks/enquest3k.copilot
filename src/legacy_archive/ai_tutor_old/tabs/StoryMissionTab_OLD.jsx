...
export default function StoryMissionTab({ weekData, recognitionRef }) {
  ...
  const handleStartMission = async (mission) => {
    ...
    // Get AI-generated greeting using tutorEngine
    try {
      const response = await runStoryMission(weekData, '', {
        storyHistory: [], // Fresh start
        turnNumber: 1
      });
      
      console.log('[StoryMission] Turn 1 Response:', response);
      
      // Set hints from AI response
      if (response.scaffold && response.scaffold.hints) {
        setCurrentHints(response.scaffold.hints);
      }
      
      const greeting = response.story_beat || response.task || "Hello! Welcome to your first day at school! I am Ms. Sarah, your teacher. What is your name?";
      addMessage({ role: 'ai', text: greeting });
      speakText(greeting);
    } catch (error) {
      console.error('Failed to generate greeting:', error);
      // Fallback
      const fallbackGreeting = "Hello! Welcome to your first day at school! I am Ms. Sarah, your teacher. What is your name?";
      addMessage({ role: 'ai', text: fallbackGreeting });
      speakText(fallbackGreeting);
      // Set fallback hints
      setCurrentHints(['My', 'name', 'is']);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const storyHistory = messages.map(m => ({
        role: m.role,
        content: m.text
      }));
      
      // Calculate current turn number
      const turnNumber = Math.floor(messages.length / 2) + 1;
      
      // Call AI provider via tutorEngine to generate response
      ...
      console.log('[StoryMission] AI Response:', response);
      ...
      // HANDLE AI FEEDBACK FOR ERRORS
      // If AI detected grammar/pronunciation issues, show feedback first
      if (response.feedback && (response.feedback.correction || response.feedback.grammar_note)) {
        const feedbackMsg = response.feedback.correction || response.feedback.grammar_note;
        addMessage({ 
          role: 'system', 
          text: `💡 Tip: ${feedbackMsg}`,
          type: 'feedback'
        });
      }
      
      // Add AI response - Show BOTH as separate messages but speak together
      let fullSpeech = '';
      
      if (response.story_beat) {
        addMessage({ role: 'ai', text: response.story_beat });
        fullSpeech = response.story_beat;
      }
      
      if (response.task) {
        addMessage({ role: 'ai', text: response.task });
        fullSpeech = fullSpeech ? `${fullSpeech} ${response.task}` : response.task;
      }
      
      // Update hints from AI response
      if (response.scaffold && response.scaffold.hints) {
        setCurrentHints(response.scaffold.hints);
      } else {
        setCurrentHints([]); // No hints = free writing
      }
      
      // Speak the complete response (both parts together)
      if (fullSpeech) {
      ...
      const minTurnsComplete = missionProgress.turnsCompleted >= currentMission.successCriteria.minTurns;
      const maxTurnsReached = missionProgress.turnsCompleted >= 20; // Increased cap to 20 for flexibility
      
      if ((minTurnsComplete && requiredWordsUsed) || maxTurnsReached) {
  ...
  // Get contextual hints for current turn (from AI response)
