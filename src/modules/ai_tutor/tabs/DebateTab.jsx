...
import { useStationProgress } from '../../../hooks/useStationProgress'; // 🔥 Progress tracking

/** Score reasoning quality by connector/evidence word usage + response length */
function scoreReasoningQuality(text) {
  const connectorBonus = /\b(because|so|but|however|therefore|although|for example|on the other hand|in my opinion|in conclusion|while|since|even though|I think|I believe)\b/i.test(text) ? 10 : 0;
  const lengthScore = Math.min(10, Math.floor(text.split(/\s+/).length / 2));
  return Math.min(10, connectorBonus + lengthScore);
}
...
const DebateTab = () => {
  ...
  const { savedData, saveProgress, markComplete } = useStationProgress(weekNumber, 'ai_debate');
  ...
  const handleSendMessage = async (userMessage) => {
    ...
    const newPhase = debateTier === 'formal' ? Math.min(debatePhase + 1, 5) : debatePhase;
    if (debateTier === 'formal' && debatePhase < 5) {
      setDebatePhase(newPhase);
    }

    // 🔥 Save debate progress
    const reasoningScore = scoreReasoningQuality(userMessage);
    const phasesCompleted = debateTier === 'formal' ? newPhase : Math.min(5, newTurnCount);
    const isDebateComplete = (debateTier === 'formal' && newPhase >= 5) || (debateTier === 'simple' && newTurnCount >= 5);
    const finalScore = Math.round(Math.min(100, (phasesCompleted / 5) * 60 + reasoningScore * 4));
    ...
    saveProgress({
      ...
      phasesCompleted,
      lastReasoningScore: reasoningScore,
      lastActivityAt: new Date().toISOString()
    }, isDebateComplete, isDebateComplete ? finalScore : 0);
    ...
    try {
      ...
      const systemPrompt = debateTier === 'formal' 
        ? `You are Nova, an ESL debate coach for young learners (ages 9-12) in a formal debate setting.
...
Challenge their thinking to help them grow!`
        : `You are Nova, an ESL teacher for young learners (ages 6-12).
        ...
        {isLoading && (
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div className="bg-white rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm">
              <p className="text-sm text-gray-500">Nova is thinking...</p>
            </div>
          </div>
        )}
