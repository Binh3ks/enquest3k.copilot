...
const WordMatch = ({ data, themeColor, isVi, onToggleLang, onReportProgress, mode }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-24">
      ...
      {isModeCompleted && !modalDismissed && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl animate-bounce-in">
            ...
            <div className="flex gap-3">
              <button onClick={() => setModalDismissed(true)} className="flex-1 py-3 bg-slate-100 text-slate-700 rounded-xl font-bold hover:bg-slate-200 transition-colors">{isVi ? 'Tiếp tục' : 'Done'}</button>
              <button onClick={() => switchGameMode(gameMode, true)} className={`flex-1 py-3 bg-${themeColor}-600 text-white rounded-xl font-bold hover:bg-${themeColor}-700 transition-colors`}>{isVi ? 'Chơi lại' : 'Play Again'}</button>
            </div>
          </div>
        </div>
      )}
