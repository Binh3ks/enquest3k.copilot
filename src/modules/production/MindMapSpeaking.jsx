          ...
          <div key={b.id} style={{ position: 'absolute', left: `${b.x}%`, top: `${b.y}%`, transform: 'translate(-50%, -50%)' }} className="z-30">
            <div style={{ borderColor: isDone ? '#22C55E' : hasError ? '#EF4444' : b.color }} 
              ...
              <div className="flex items-center gap-2 font-black text-slate-800 uppercase text-xs italic">
                <div style={{ backgroundColor: b.color }} className="p-1.5 rounded-lg text-white shadow-sm cursor-pointer hover:scale-110 transition-transform" onClick={() => speakText(b.display, b.audioUrl, 1.0, null, 'mindmap_speaking', parseInt(weekId), 'advanced', true)}> {/* ⚡ instant mode - b.display matches audio file content */}
                  <Volume1 size={14}/>
                </div>
