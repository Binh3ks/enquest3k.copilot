...
import { recordReview, makeWordId, getDueToday } from '../../utils/wordMemoryBank';
import { useUserStore } from '../../stores/useUserStore';

// Thêm prop onWeekComplete
const ReviewDashboard = ({ themeColor, reviewItems, setReviewItems, onWeekComplete }) => {
    ...
    const handleCompleteDrill = (completedId) => {
        ...
        // Feed word drills into SRS bank so review history influences future scheduling
        const doneItem = reviewItems.find(item => item.id === completedId);
        if (doneItem && doneItem.type === 'Word') {
            try {
                const wid = makeWordId(doneItem.week, doneItem.answer);
                recordReview(wid, true); // student answered correctly to complete drill
            } catch { /* word may not be in bank yet — ignore */ }
        }
        ...
        if (isComplete) {
            markComplete(100);
            // Clear SRS badge: sweep all remaining due words so sidebar badge goes to 0
            try {
                const stillDue = getDueToday(200);
                stillDue.forEach(entry => recordReview(entry.word_id, true));
            } catch { /* ignore */ }
        }
    }
