...
const ReadingExplore = ({ data, themeColor, isVi, onToggleLang, onReportProgress }) => {
  ...
  // 🔥 Week 16+: Detect new dual-tab structure (read_stem + read_social)
  const isW16Plus = data && (data.read_stem || data.read_social);
  if (isW16Plus) {
  ...
  useEffect(() => {
    if (data && data.content_en) {
      ...
      // 🚀 Pre-cache full reading content for instant playback (ONCE per data load)
      if (data.content_en && !hasPrefetched.current) {
        hasPrefetched.current = true;
        console.log('[ReadingExplore] � DEBUG - content:', {
          hasContent: !!data.content_en,
          contentLength: data.content_en.length,
          sentencesCount: combined.length
        });
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden relative group">
        ...
        <div className="p-8">
            <div className="flex justify-between items-start mb-6">
               <button onClick={() => speakText(renderCleanText(data.content_en), data.audio_url || null, 1.0, null, 'read', parseInt(weekId), learningMode)} className={`p-3 bg-${themeColor}-600 text-white rounded-full shadow-lg hover:bg-${themeColor}-700 transition-transform hover:scale-110 flex-shrink-0`}>
