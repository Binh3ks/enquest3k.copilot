...
const GrammarEngine = ({ data, themeColor, isVi, onToggleLang, onReportProgress }) => {
  ...
  const checkAnswer = useCallback(() => {
    ...
    const effectiveAnswer = currentQ.answer || currentQ.correct || '';
    const result = analyzeAnswer(cleanInput, effectiveAnswer, checkMode);
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in pb-24">
            ...
            <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4 bg-white animate-slide-down">
              {data.grammar_explanation.rules.map((rule, idx) => (
                <div key={idx} className={`p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-all hover:border-${themeColor}-200 bg-slate-50/50`}>
                  ...
                  {(rule.example_en || rule.example) && <p className="text-xs text-slate-500 italic border-t border-slate-200 pt-2 mt-2">Ex: {renderRichText(isVi ? (rule.example_vi || rule.example_en || rule.example) : (rule.example_en || rule.example))}</p>}
                </div>
              ))}
            </div>
          )}
      ...
      <div className={`card-3d p-6 md:p-8 border-${themeColor}-100 min-h-[300px] flex flex-col`}>
        <div className="mb-8">
          ...
          {(currentQ.type === 'fill' || currentQ.type === 'fill_blank') && (currentQ.hint || currentQ.hint_vi) && (
          ...
          {currentQ.type === 'unscramble' && (() => {
            const qText = currentQ.question || currentQ.question_en || currentQ.sentence || '';
            const unscrambleWords = currentQ.words ||
              (qText.match(/\[([^\]]+)\]/)?.[1]
                ?.split('/')
                .map(w => w.trim())
                .filter(Boolean)) || [];
            return unscrambleWords.length > 0 ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {unscrambleWords.map((w, i) => (
