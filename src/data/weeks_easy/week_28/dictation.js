export default {
  title: "Dictation: The Transport Race",
  image_url: "/images/week28/read_cover_w28.jpg",
  content_en: `Once upon a time, a **slow hare** was very proud and fast. He always bragged about **winning the race** and laughed at the slow tortoise. One day, the slow tortoise smiled calmly and said, 'Slow and steady wins the race.' The slow hare got on his **fast bicycle** while the slow tortoise got on his **small boat**. BANG — the race began suddenly!\n\nThe slow hare got ahead of everyone, but then he got tired and **fell asleep** under a big shady tree. Meanwhile, the slow tortoise kept on sailing slowly but never stopped and never gave up. He **crossed the river** and **kept going** to the **finish line**.\n\nAll the forest animals watched from the riverbank. They could not believe the slow tortoise was winning the race!\n\nWhen the slow hare woke up, the slow tortoise had already **won the race**. All the animals cheered loudly.\n\nSlow and steady wins the race.`,
  content_vi: `Ngày xửa ngày xưa, một con thỏ rất tự hào và nhanh nhẹn. Nó luôn khoe khoang về việc thắng mọi cuộc đua và nhìn vào con rùa chậm chạp một cách cười nhạo. Một ngày nọ, con rùa mỉm cười và nói, 'Chậm mà chắc sẽ thắng cuộc đua.' Con thỏ lên xe đạp trong khi con rùa lên thuyền. BÙM — cuộc đua bắt đầu. Con thỏ vượt lên trước mọi người nhưng rồi nó mệt và ngủ thiếp đi dưới một gốc cây râm mát. Trong khi đó, con rùa tiếp tục bơi chậm nhưng không bao giờ dừng lại và không bao giờ bỏ cuộc. Nó bơi qua sông và tiếp tục đi đến vạch đích. Tất cả các con vật trong rừng theo dõi từ bờ sông. Họ không thể tin con rùa đang thắng cuộc đua. Khi con thỏ tỉnh dậy, con rùa đã ở đó rồi. Con rùa thắng cuộc đua. Tất cả các con vật hoan hô to. Chậm mà chắc sẽ thắng cuộc đua.`,
  audio_url: "/audio/week28_easy/dictation_main.mp3",
  sentences: [
    { id: 1, text: "Once upon a time, a **slow hare** was very proud and fast.", audio_url: "/audio/week28_easy/dictation_s1.mp3" },
    { id: 2, text: "He always bragged about **winning the race** and laughed at the slow tortoise.", audio_url: "/audio/week28_easy/dictation_s2.mp3" },
    { id: 3, text: "The slow hare got on his **fast bicycle** while the slow tortoise got on his **small boat**.", audio_url: "/audio/week28_easy/dictation_s3.mp3" },
    { id: 4, text: "But then he got tired and **fell asleep** under a big shady tree.", audio_url: "/audio/week28_easy/dictation_s4.mp3" },
    { id: 5, text: "He **crossed the river** and **kept going** to the **finish line**.", audio_url: "/audio/week28_easy/dictation_s5.mp3" },
    { id: 6, text: "They could not believe the slow tortoise was winning the race!", audio_url: "/audio/week28_easy/dictation_s6.mp3" },
    { id: 7, text: "When the slow hare woke up, the slow tortoise had already **won the race**.", audio_url: "/audio/week28_easy/dictation_s7.mp3" },
    { id: 8, text: "Slow and steady wins the race.", audio_url: "/audio/week28_easy/dictation_s8.mp3" }
  ]
};
