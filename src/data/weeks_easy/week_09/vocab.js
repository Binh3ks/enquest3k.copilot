export default {
    vocab: [
    {
      id: 1,
    word: "city",
    pronunciation: "/ˈsɪt.i/",
    definition_en: "thành phố", definition_vi: "thành phố",
    definition_en: "a big town",
    example: "I live in a big city.",
    collocation: ["big city", "busy city"],
    image_url: "/images/week09_easy/city.jpg",
    audio_word: "/audio/week9_easy/vocab_city.mp3",
    audio_example: "/audio/week09/vocab_ex_city.mp3"
  },
    {
    id: 2,
    word: "street",
    pronunciation: "/striːt/",
    definition_en: "đường phố", definition_vi: "đường phố",
    definition_en: "a road in a city",
    example: "I walk on a busy street.",
    collocation: ["busy street", "on the street"],
    image_url: "/images/week09_easy/street.jpg",
    audio_word: "/audio/week9_easy/vocab_street.mp3",
    audio_example: "/audio/week09/vocab_ex_street.mp3"
  },
    {
    id: 3,
    word: "noisy",
    pronunciation: "/ˈnɔɪ.zi/",
    definition_en: "ồn ào", definition_vi: "ồn ào",
    definition_en: "very loud",
    example: "The city is a noisy place.",
    collocation: ["noisy place", "noisy city"],
    image_url: "/images/week09_easy/noisy.jpg",
    audio_word: "/audio/week9_easy/vocab_noisy.mp3",
    audio_example: "/audio/week09/vocab_ex_noisy.mp3"
  },
    {
    id: 4,
    word: "busy",
    pronunciation: "/ˈbɪz.i/",
    definition_en: "đông đúc", definition_vi: "đông đúc",
    definition_en: "full of people",
    example: "The busy city has many cars.",
    collocation: ["busy city", "busy street"],
    image_url: "/images/week09_easy/busy.jpg",
    audio_word: "/audio/week9_easy/vocab_busy.mp3",
    audio_example: "/audio/week09/vocab_ex_busy.mp3"
  },
    {
    id: 5,
    word: "tall",
    pronunciation: "/tɔːl/",
    definition_en: "cao", definition_vi: "cao",
    definition_en: "very high",
    example: "I see a tall building.",
    collocation: ["tall building", "tall tree"],
    image_url: "/images/week09_easy/tall.jpg",
    audio_word: "/audio/week9_easy/vocab_tall.mp3",
    audio_example: "/audio/week09/vocab_ex_tall.mp3"
  },
    {
    id: 6,
    word: "modern",
    pronunciation: "/ˈmɑː.dɚn/",
    definition_en: "hiện đại", definition_vi: "hiện đại",
    definition_en: "new and nice",
    example: "The modern city has tall buildings.",
    collocation: ["modern city", "modern building"],
    image_url: "/images/week09_easy/modern.jpg",
    audio_word: "/audio/week9_easy/vocab_modern.mp3",
    audio_example: "/audio/week09/vocab_ex_modern.mp3"
  },
    {
    id: 7,
    word: "car",
    pronunciation: "/kɑːr/",
    definition_en: "ô tô", definition_vi: "ô tô",
    definition_en: "a vehicle with four wheels",
    example: "I see a fast car on the street.",
    collocation: ["fast car", "red car"],
    image_url: "/images/week09_easy/car.jpg",
    audio_word: "/audio/week9_easy/vocab_car.mp3",
    audio_example: "/audio/week09/vocab_ex_car.mp3"
  },
    {
    id: 8,
    word: "bus",
    pronunciation: "/bʌs/",
    definition_en: "xe buýt", definition_vi: "xe buýt",
    definition_en: "a big vehicle for many people",
    example: "I ride the school bus.",
    collocation: ["school bus", "ride the bus"],
    image_url: "/images/week09_easy/bus.jpg",
    audio_word: "/audio/week9_easy/vocab_bus.mp3",
    audio_example: "/audio/week09/vocab_ex_bus.mp3"
  },
    {
    id: 9,
    word: "building",
    pronunciation: "/ˈbɪl.dɪŋ/",
    definition_en: "tòa nhà", definition_vi: "tòa nhà",
    definition_en: "a house or tall structure",
    example: "The tall building is very big.",
    collocation: ["tall building", "big building"],
    image_url: "/images/week09_easy/building.jpg",
    audio_word: "/audio/week9_easy/vocab_building.mp3",
    audio_example: "/audio/week09/vocab_ex_building.mp3"
  },
    {
    id: 10,
    word: "traffic",
    pronunciation: "/ˈtræf.ɪk/",
    definition_en: "giao thông", definition_vi: "giao thông",
    definition_en: "cars and buses on roads",
    example: "The city has heavy traffic.",
    collocation: ["heavy traffic", "busy traffic"],
    image_url: "/images/week09_easy/traffic.jpg",
    audio_word: "/audio/week9_easy/vocab_traffic.mp3",
    audio_example: "/audio/week09/vocab_ex_traffic.mp3"
  }
  ]
};
