...
def process_folder_by_prompt(folder_name, prompt_file):
    ...
    # Hỗ trợ 2 patterns:
    #   1. "N_N_blahblah.png" → số thứ tự lấy từ số thứ 2 (vd: 1_1_ → 1, 1_11_ → 11, 10_20_ → 20)
    #   2. "download.png" → số thứ tự 1, "download(1).png" → số thứ tự 2, "download(2).png" → số thứ tự 3
    file_map = {}
    for fname in image_files:
        idx = None
        
        # Pattern 1: Prefix với 2 số (vd: 1_1_..., 1_11_..., 13_13_..., 10_20_...)
        # Lấy số thứ HAI (là số thứ tự thực sự của prompt)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Sử dụng: python auto_rename.py [Số tuần] [--folder TÊN_THƯ_MỤC]")
        print("Ví dụ: python auto_rename.py 6")
        print("       python auto_rename.py 28 --folder week28_new")
        sys.exit(1)
    ...
    # Nếu có --folder, xử lý folder đó với prompt advanced
    if custom_folder:
        process_folder_by_prompt(custom_folder, prompt_adv)
        sys.exit(0)
