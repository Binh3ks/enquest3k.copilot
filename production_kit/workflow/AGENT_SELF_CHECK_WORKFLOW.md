...
- [ ] ✅ **CLONE** golden standards (Week 7 AI Tutor + Week 6 Stations for W1-15, Week 16 Stations for W16+) - NOT generate from scratch
...
```bash
# Verify Week 7 AI Tutor + station golden standards exist
ls -la src/data/weeks/week_07_real.js || { echo "❌ Week 7 AI Tutor missing"; exit 1; }
# W1-15: use Week 6; W16+: use Week 16
ls -la src/data/weeks/week_06/index.js || { echo "❌ Week 6 Advanced missing (W1-15 golden standard)"; exit 1; }
ls -la src/data/weeks_easy/week_06/index.js || { echo "❌ Week 6 Easy missing (W1-15 golden standard)"; exit 1; }
ls -la src/data/weeks/week_16/index.js || { echo "❌ Week 16 Advanced missing (W16+ golden standard)"; exit 1; }
ls -la src/data/weeks_easy/week_16/index.js || { echo "❌ Week 16 Easy missing (W16+ golden standard)"; exit 1; }
...
- [ ] I have read **Blueprint Section VII.b (COLLOCATIONS/CHUNKS)** as the FIRST authoring principle?
- [ ] I have read rest of Blueprint to understand app philosophy & content rules?
- [ ] I have read Syllabus for Week N theme, grammar, vocabulary?
- [ ] Week 7 (`week_07_real.js`) exists for AI Tutor template?
- [ ] Week 6 (`weeks/week_06/*.js`) exists for Stations template (W1-15)?
- [ ] Week 16 (`weeks/week_16/*.js`) exists for Stations template (W16+)?
- [ ] I understand Phase (1/2/3) for Week N?

#### Actions:

**Step 0.1: READ BLUEPRINT — Section VII.b FIRST (CHUNK-FIRST AUTHORING)**
```bash
# 🔴 CRITICAL: Section VII.b is THE FOUNDATION — read it FIRST before ANY content
# File: production_kit/reference/ENGQUEST_BLUEPRINT_DETAILED_SPECS_V5.0.md

# 1. Read Section VII.b (COLLOCATIONS/CHUNKS) — THE CORE PRINCIPLE
grep -A 80 "VII.b" "production_kit/reference/ENGQUEST_BLUEPRINT_DETAILED_SPECS_V5.0.md" | head -85

# Key rules (MUST FOLLOW):
# - Single-word bolds = 0 allowed in read.js (ALL weeks!)
# - W28+: embed ≥10 chunks per passage, ≥1 chunk per sentence
# - W28+: 15-18 sentences, 180-220 words
# - Every sentence → embed 1-3 natural chunks from the checklist
# - dictation/shadowing → select chunk-rich sentences from read.js

# Chunk embedding checklist (use when writing sentences):
# Movement:      went to, ran to, flew to, drove to, walked to, climbed up, ran away, ran down
# Daily routine: got up, fell asleep, got dressed, woke up, put on, took off, brushed teeth
# Action:        looked at, looked for, waited for, picked up, gave up, kept on, found out
# Communication: talked to, listened to, looked after, shouted at, smiled at, waved to
# Mental state:  felt happy, felt sad, got angry, got tired, got surprised
# Accumulated:   made up a story, played games, read books, hurt my knee, fell down, built a house

# 🔴 CRITICAL: A bold `**...**` is a FALSE chunk if it is:
# - Single word (e.g. **firefighters**) — forbidden W28+
# - Adj + Profession (e.g. **friendly pilot**) — unnatural
# - "the" + Noun (e.g. **the dolphins**) — article+noun is not a chunk
# - Verb + Pronoun (e.g. **gave it/her/them**) — not a fixed unit
# - Generic Verb + Prep (e.g. **went to**, **came to**) — just movement
# - Verb + Adverb (e.g. **began suddenly**, **cheered loudly**) — generic
# - Verb + Adj + Noun (e.g. **brought fresh strawberries**) — only the Adj+Noun
#   is the real chunk; the verb makes it a free grammatical phrase
# - Wh-clause fragment (e.g. **remember this adventure**) — not a unit
#
# Tooling: see production_kit/tools/audit_false_chunks.py and
# production_kit/tools/fix_false_chunks.py for automated scanning.

# 2. Then read rest of Blueprint for other rules:
...
# - Theme: ???
# - Grammar: ???
# - Advanced Vocabulary (Tier 2/3): W1-15=10 words, W16-27=13 words, W28+=18 words
# - Easy Vocabulary (Tier 1): same count as Advanced
```
...
ls -la src/data/weeks/week_06/*.js  # W1-15 station template
ls -la src/data/weeks_easy/week_06/*.js
ls -la src/data/weeks/week_16/*.js  # W16+ station template (logic_science.js)
ls -la src/data/weeks_easy/week_16/*.js
```

**Step 0.4: Determine Phase**
```bash
# 3. Check Phase (determines vocab/word_power/logic counts)
# Phase 1: Weeks 1-54
#   - vocab: W1-15=10, W16-27=13, W28+=18
#   - word_power: W1-27=6 items, W28+=8 items (phrasal verbs/idioms)
#   - logic_science: 3-7 questions (flexible, per Blueprint)
# Phase 2: Weeks 55-120 (word_power=8 items)
# Phase 3: Weeks 121-156 (word_power=10 items, 10 logic questions)
```

#### Self-Check Before Proceeding:
- [ ] I have read **Blueprint Section VII.b (COLLOCATIONS/CHUNKS)** as the first authoring principle
- [ ] I have read rest of Blueprint and understand app architecture
...
- [ ] I understand **chunk-first authoring**: every sentence embeds ≥1 natural chunk; bold ONLY chunks (no single-word bolds)
...
#### POST-MODE VERIFICATION (sau khi xong tất cả 17 files của 1 mode):
```bash
# Đếm tổng files:
ls -1 src/data/weeks/week_N/*.js | wc -l      # PHẢI = 17
ls -1 src/data/weeks_easy/week_N/*.js | wc -l  # PHẢI = 17
...
// 5. Replace vocabulary (W1-15=10, W16-27=13, W28+=18 objects - use Tier 2/3 words from Syllabus)
...
**Time: ~20 minutes**  
**Purpose: Clone schema from Week 16 (W16+) or Week 6 (W1-15), replace content**

#### Actions:

**Step 3.1: Clone all 16 station files + index.js (17 total)**
```bash
# ⚠️ CRITICAL: Use Week 16 as template for W16+, Week 6 for W1-15
TEMPLATE_WEEK=16  # change to 6 for W1-15

for file in vocab word_power read explore dictation shadowing grammar word_match ask_ai mindmap writing daily_watch games; do
  cp "src/data/weeks/week_${TEMPLATE_WEEK}/$file.js" "src/data/weeks/week_N/$file.js"
  echo "Cloned: $file.js"
done

# W16+ uses logic_science.js, W1-15 uses logic.js
if [ "$TEMPLATE_WEEK" -eq 16 ]; then
  cp "src/data/weeks/week_16/logic_science.js" "src/data/weeks/week_N/logic_science.js"
  cp "src/data/weeks/week_16/singapore_math.js" "src/data/weeks/week_N/singapore_math.js"
  echo "Cloned: logic_science.js + singapore_math.js (W16+)"
  # ✅ Bar model naming convention (W34 Lesson): bar_model path MUST be:
  # /images/weekN/barmodel_w{N}_{mode}_p{n}_v{n}.jpg
  # Example: /images/week34/barmodel_w34_adv_p1_v1.jpg
  # ❌ WRONG: barmodel_q1.png — generator would create with wrong name
  python3 tools/generate_logiclab_barmodels.py N
  node tools/validate_barmodels.js N  # ← MANDATORY — DO NOT SKIP
else
  cp "src/data/weeks/week_06/logic.js" "src/data/weeks/week_N/logic.js"
  echo "Cloned: logic.js (W1-15)"
fi

# ⚠️ CRITICAL (Week 13 Lesson): Also clone index.js!
cp "src/data/weeks/week_${TEMPLATE_WEEK}/index.js" "src/data/weeks/week_N/index.js"
...
**vocab.js:**
- [ ] W1-15: 10 words | W16-27: 13 words | W28+: 18 words (Tier 2/3 from Syllabus)
...
- [ ] **Word count by phase** (target range, NOT hard limit):
  - W01-15: 85-135 words | W16-27: 145-215 words | W28+: **180-220 words**
- [ ] **Sentence count**: Advanced 11-18 sentences | Easy 8-14 sentences
- [ ] **🔴 CHUNK-FIRST ENFORCEMENT (MUST PASS — CHECK 20c)**
  - **W28+: Minimum 10 bolded multi-word chunks per passage, 1-3 per sentence**
  - **W1-27: Minimum bolded chunks embedded (policy: ≥10 for W16+, ≥0 for W1-15)**
  - **ALL WEEKS: Single-word bolds = 0 allowed**
  - **EVERY sentence must embed ≥1 natural chunk** from the checklist (see Step 0.1)
  - Bold pattern: ✅ `**ran down** the hill` | ❌ `ran down **the** hill` | ❌ `**ran** down the hill`
  - Run CHECK 20c: `bash production_kit/tools/code_quality_gate.sh N 2>&1 | grep -A5 "CHECK 20c"`
  - **🔴 IF CHECK 20c FAILS → STOP. Fix sentences. Do not proceed.**
- [ ] W1-15: ≥10 bold words | W16+: ≥13 bold words: `**word**` format
- [ ] `comprehension_questions` field (NOT `check_questions`) — W31+ explore.js uses `check_questions`
- [ ] Syntax validation PASSED
- [ ] **🔴 CHECK 20d (period count matching):**
  - read.js period count (sentences ending with `.`) must match dictation sentence count
  - Em-dash continuations (`— text!`) do NOT add extra periods
  - Run: `node -e "require('./src/data/weeks/week_N/read.js')"` then count periods from parsed content
  - dictation.js must have same or fewer sentences (selected subset)

**dictation.js:**
- [ ] **SELECTED SUBSET from read.js** (not all sentences, not self-created)
- [ ] **W1-27:** All sentences from read.js (8-15 sentences)
- [ ] **W28+:** 10-12 sentences (Advanced) / 8-10 sentences (Easy) — selected for collocation coverage
- [ ] Sentences chosen to cover: all vocab words + key collocations + story arc
- [ ] Each item has `audio_url` field
- [ ] **`content_en` EXACTLY matches read.js content_en** (including all `**` bold markers and `\n` newlines)
- [ ] **🔴 CHECK 42 (dictation sentences verbatim from read.js):**
  - Every dictation sentence must be a verbatim substring of read.js `content_en`
  - Read.js often continues sentences after em-dash (`—`) — dictation MUST include full continuation
  - Example: read.js `"They hammered the nails carefully and put the birdhouse..."` → dictation must include `"They hammered the nails carefully and put..."`, NOT just `"They put the birdhouse..."`
  - CHECK 42 uses `strip_bold()` on BOTH sides, so `**bold**` in dictation sentences MATCHES correctly
  - Run: `bash production_kit/tools/code_quality_gate.sh N` (CHECK 42 section)

**shadowing.js:**
- [ ] **W1-15:** Phrase chunks (3-5 words per block) for pronunciation training
- [ ] **W16-27:** Full sentences, gradually increasing to 15-17 sentences
- [ ] **W28+:** Full sentences, **minimum 8**, count increases gradually; focus on collocation-rich sentences
- [ ] Each item has `audio_url` field
- [ ] `shadowing.js` has `audio_full` field at root
- [ ] **`content_en` EXACTLY matches read.js content_en** (including all `**` bold markers and `\n` newlines)
- [ ] **🔴 CHECK 42 (shadowing sentences verbatim from read.js):** Same rule as dictation — every shadowing sentence must be a verbatim substring of read.js `content_en`
...
grep -c '"question_en":' src/data/weeks/week_N/grammar.js
...
**🚨 MANDATORY VALIDATION GATE 1: Content Theme + Chunk-First Check (Week 15 Lesson)**
...
# Test 4: 🔴 CHUNK-FIRST (CHECK 20c) — run full quality gate
echo "Test 4: Chunk-first enforcement (CHECK 20c)"
bash production_kit/tools/code_quality_gate.sh N 2>&1 | grep -E "CHECK 20c|20c.*chunk|20c.*colloc|FAIL|PASS|chunk"
CHECK20C_RESULT=$?
if [[ $CHECK20C_RESULT -ne 0 ]]; then
  echo "❌ FAILED: CHECK 20c (chunk-first) — single-word bolds or insufficient chunks"
  echo "   Run full gate to see details:"
  echo "   bash production_kit/tools/code_quality_gate.sh N"
  echo ""
  echo "   Common fix: Replace single-word bolds with natural chunk patterns:"
  echo "   ❌ The hare ran **very** fast. → ✅ The hare **ran very fast**."
  echo "   ❌ He was **proud**. → ✅ He was **proud of being fast**."
  exit 1
fi
echo "✅ CHECK 20c PASSED (chunk-first enforcement OK)"
...
**Step 5: Validate Video IDs (MANDATORY — use validation script)**
```bash
# Validate all video thumbnails return 200 OK (NEW — W34 Lesson)
node tools/validate_video_thumbnails.js N

# ❌ If FAILS: Re-run update_videos.js or find replacement video IDs
# NEVER commit with fake/placeholder video IDs
```

#### Self-Check:
- [ ] Found 5 videos related to Week N theme
- [ ] All video IDs are valid (thumbnails load) — checked by validate_video_thumbnails.js
...
- [ ] **Updated BOTH modes**: `week_N/daily_watch.js` and `week_N_easy/daily_watch.js`
- [ ] Required fields present: `category`, `weekId`, `videoId`, `title`, `duration`, `sim_duration`, `thumb`
...
**Validation Gate 4: Chunk & Collocation Audit (MUST PASS)**
```bash
# Run automated chunk audit (catches adj+profession, the+noun, verb+adverb, etc.)
echo "=== CHUNK AUDIT ==="
python3 production_kit/tools/audit_false_chunks.py $N
# Expected: "No false chunks found."
# If false chunks found: auto-fix with --apply, OR edit manually per the rules in Step 0.1

# Optional: auto-fix (review diff after!)
# python3 production_kit/tools/fix_false_chunks.py $N --apply

# Required for every NEW week production (May 2026+)
# Reference: production_kit/tools/audit_false_chunks.py
# Reference: production_kit/tools/fix_false_chunks.py
```
...
**Step 4.1: Clone Easy templates + index.js (17 total)**
```bash
# ⚠️ CRITICAL: Use Week 16 Easy as template for W16+, Week 6 Easy for W1-15
TEMPLATE_WEEK=16  # change to 6 for W1-15

for file in vocab word_power read explore dictation shadowing grammar word_match ask_ai mindmap writing daily_watch games; do
  cp "src/data/weeks_easy/week_${TEMPLATE_WEEK}/$file.js" "src/data/weeks_easy/week_N/$file.js"
  echo "Cloned easy: $file.js"
done

# W16+ uses logic_science.js, W1-15 uses logic.js
if [ "$TEMPLATE_WEEK" -eq 16 ]; then
  cp "src/data/weeks_easy/week_16/logic_science.js" "src/data/weeks_easy/week_N/logic_science.js"
  cp "src/data/weeks_easy/week_16/singapore_math.js" "src/data/weeks_easy/week_N/singapore_math.js"
  echo "Cloned easy: logic_science.js + singapore_math.js (W16+)"
  # Bar model images are generated once (both modes) — no need to re-run for EASY
else
  cp "src/data/weeks_easy/week_06/logic.js" "src/data/weeks_easy/week_N/logic.js"
  echo "Cloned easy: logic.js (W1-15)"
fi

# ⚠️ CRITICAL (Week 13 Lesson): Also clone index.js!
cp "src/data/weeks_easy/week_${TEMPLATE_WEEK}/index.js" "src/data/weeks_easy/week_N/index.js"
...
#### Self-Check:
- [ ] All 16 Easy station files + index.js created (17 total)
- [ ] Easy vocabulary = Tier 1 (concrete actions: sing, dance, run) — same count as Advanced
- [ ] Easy read.js = Personal context ("I have...", "My name is...")
- [ ] Advanced read.js = Formal context ("Today is...", "There is...")
- [ ] dictation.js = **SELECTED SUBSET** from Easy read.js (not all sentences, not self-created)
- [ ] shadowing.js = **SELECTED SUBSET** from Easy read.js, minimum 8 sentences (W28+)
- [ ] grammar.js = EXACTLY 20 exercises + instruction section (both modes)
- [ ] **🔴 VALIDATION (Lỗi tiềm ẩn #3):**
```bash
# Both modes MUST have 20 exercises
grep -c '"question_en":' src/data/weeks/week_N/grammar.js          # = 20
grep -c '"question_en":' src/data/weeks_easy/week_N/grammar.js    # = 20
```
- [ ] **🔴 VALIDATION: Bold words count:**
```bash
# W1-15: ≥10 bold words | W16+: ≥13 bold words
grep -o '\*\*[^*\n]\+\*\*' src/data/weeks/week_N/read.js | wc -l  # ≥13 for W16+
```
- [ ] **🔴 VALIDATION: Dictation subset (W28+):**
```bash
# W1-27: dictation sentences = read sentences (all)
# W28+: dictation 10-12 sentences (Advanced), 8-10 (Easy) — subset
```
- [ ] **🔴 VALIDATION: Collocation density (W28+):**
```bash
# Count collocations in read.js: grep for common collocation patterns
# Target: ≥10 collocations per passage (W28+)
bash production_kit/tools/code_quality_gate.sh N 2>&1 | grep -E "CHECK 20c|20c.*chunk|FAIL|PASS"
# MUST PASS — single-word bolds = 0 allowed, ≥10 multi-word chunks required (W28+)
```
- [ ] **🔴 VALIDATION: explore.js `check_questions` for W31+:**
```bash
# W1-30: use comprehension_questions
# W31+: use check_questions (different schema — NOT comprehension_questions!)
grep "check_questions" src/data/weeks_easy/week_N/explore.js  # → must exist for W31+
grep "comprehension_questions" src/data/weeks_easy/week_N/explore.js  # → must be EMPTY for W31+
```
- [ ] **🔴 VALIDATION: video_tasks.json dual entries:**
```bash
# Every week needs BOTH Advanced and Easy entries
grep "\"weekId\".*: $N" src/data/video_tasks.json      # → must exist
grep "\"weekId\".*: \"${N}_easy\"" src/data/video_tasks.json  # → must exist
```
- [ ] **🔴 VALIDATION: dictation/shadowing CHECK 42 (verbatim from read.js):**
```bash
# Run the full quality gate — this is the authoritative check
bash production_kit/tools/code_quality_gate.sh N
# Key checks: C-42 (dictation/shadowing verbatim), C-34 (vocab images), C-20d (period count)
```
- [ ] **(W13)** index.js voiceConfig has 5 DISTINCT voices
- [ ] **(W13)** word_power: W1-27=6 items, W28+=8 items (phrasal verb/idiom structure, NO collocations sub-field)
...
# EXPECTED COUNTS (Phase 1 — varies by sub-period):
# Advanced: ~163-183 files breakdown (W1-15 / W16-27 / W28+):
#   - vocab: W1-15=40(10×4), W16-27=52(13×4), W28+=72(18×4)
#   - word_power: W1-27=30(6×5), W28+=40(8×5)
#   - read: 1
#   - grammar: 20 (both modes)
#   - logic_science: 3-7 (flexible per Blueprint)
#   - dictation: W1-27=all read sentences; W28+=10-12 Adv / 8-10 Easy (selected subset)
#   - shadowing: W1-15=phrase chunks; W16-27=full sentences; W28+=min 8 full sentences
#   - ask_ai: 5
#   - mindmap: 20  
#   - daily_watch: 7
#   - + other stations
...
**🔴 image_prompts.txt format check (CRITICAL):**
```bash
# MUST use Filename: format — NOT markdown ### N. filename.jpg
# Correct:
grep -c "Filename:" Production_FINAL/IMAGE\ PROMPTS/week_NN_image_prompts.txt
# → must equal number of entries (not 0)

# Incorrect (will fail CHECK 34):
grep -c "###" Production_FINAL/IMAGE\ PROMPTS/week_NN_image_prompts.txt
# → should be 0 for correct format
```

Correct format example:
```
1. Hãy tạo hình ảnh sau đây. Filename: vocab_tidy.jpg. A cozy child's bedroom... No text or letters in the image.
2. Hãy tạo hình ảnh sau đây. Filename: vocab_birdhouse.jpg. A small wooden birdhouse... No text or letters in the image.
```

**If file uses markdown format (`### N. filename.jpg`) → REWRITE immediately using `Filename:` format.**
...
- ✅ **(W13)** Create index.js for BOTH modes (17 files per mode)
...
- ❌ Truncate dictation/shadowing sentences at em-dash — must include full read.js text (CHECK 42)
- ❌ Use `comprehension_questions` in W31+ explore.js — must use `check_questions`
- ❌ Use markdown format in image_prompts.txt — must use `Filename:` format
...
# W31+ explore.js must use check_questions (NOT comprehension_questions)
node --input-type=module <<'XCHK'
if (N >= 31) {
  const m = await import('./src/data/weeks/week_N/explore.js');
  if (m.default.check_questions) {
    console.log('✅ W31+ explore.js uses check_questions');
  } else if (m.default.comprehension_questions) {
    console.error('❌ W31+ explore.js uses WRONG field: comprehension_questions (should be check_questions)');
  }
}
XCHK

# CHECK 42: dictation/shadowing content_en must match read.js exactly
node --input-type=module <<'C42CHK'
const read = (await import('./src/data/weeks/week_N/read.js')).default;
const dict = (await import('./src/data/weeks/week_N/dictation.js')).default;
const shad = (await import('./src/data/weeks/week_N/shadowing.js')).default;
if (dict.content_en !== read.content_en) {
  console.error('❌ dictation.js content_en does NOT match read.js content_en');
  console.error('   Lengths: dictation=' + dict.content_en.length + ' read=' + read.content_en.length);
} else {
  console.log('✅ dictation.js content_en matches read.js');
}
if (shad.content_en !== read.content_en) {
  console.error('❌ shadowing.js content_en does NOT match read.js content_en');
} else {
  console.log('✅ shadowing.js content_en matches read.js');
}
C42CHK
```
...
Summary:
- Files Created: 35 (1 AI Tutor + 16 Advanced + 16 Easy + 2 index.js)
- Audio Files: [EXACT_COUNT] Advanced + [EXACT_COUNT] Easy = [TOTAL]
  - Expected: W1-15=163 ADV/157 EAS, W16-27=175+/169+, W28+=195+/189+ (Phase 1)
...
- Grammar Count: 20 exercises per mode ✅ (both Advanced + Easy)
